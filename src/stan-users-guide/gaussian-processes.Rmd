...
```
Where since $\eta \sim normal(0, 1)$, $f$ is distributed multi-variate normally. 
...
The dot product kernel, or linear kernel, is analogous to a Bayesian linear regression with a normal(0, 1) prior on regression coefficients. This can be combined with other covariance functions to 
