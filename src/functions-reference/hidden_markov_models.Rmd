...
$$
\omega_{k n} = p(y_n \mid x_n = k, \phi).
$$
...
*   *`log_omega`*: $\log p(y_n \mid x_n = k, \phi)$, log density of each output,

*   *`Gamma`*: $\Gamma_{ij} = p(x_n = j | x_{n - 1} = i, \phi)$, the transition matrix,

*   *`rho`*: $p(x_0 = k \mid \phi)$, the initial state probability.
...
Returns an array of integers over $\{1, ..., K\}$,
...
The $i^\mathrm{th}$ column is a simplex of probabilities for the $n^\mathrm{th}$ variable.
