...
Equivalently, we can use the posterior predictive equations for Gaussian process regression:
...
Equivalently, we can use the predictive equations for the multivariate gaussian distribution:
```
functions {
...
generated quantities {
  vector[N_pred] f_pred = gp_pred_rng(x_pred, y, x, tau, l, sigma);
  vector[N_pred] y_pred;
