...
numerical solution. Specifically, when solving an
$n$-dimensional ODE with unknowns $y=(y_1,\dots,y_n)^T$, the
solver controls estimated local error $e=(e_1,\dots,e_n)^T$ at
certain step through its weighted root-mean-square norm $\|\cdot\|$

$$
\|e\| = \sqrt{\sum_{i=1}^n{\frac{e_i^2}{\text{relative\_tolerance}\times y_i + \text{absolute\_tolerance}}}} < 1
$$
<a name="id:ode-local-error.equation"></a>

The solver will reduce the stepsize when the above test fails.

From the test one can see the role of tolerances by assuming different scales of
$y_i$. On one hand, the absolute tolerance has little effect when $y_i \gg 1$, on the other the relative tolerance can
not affect the norm when $y_i = 0$. It is strongly encouraged choosing the
tolerance carefully according to the ODE and its application. One can follow
@Brenan:1996 for a rule of thumb:
let $m$ be the number of significant digits required for $y$, one can set
$\text{relative\_tolerance}=10^{-(m+1)}$, and set absolute tolerance at
which $y$ becomes insigfinicant. Note that the same weighted root-mean-square norm
is used to control nonlinear solver convergence,  and the same
