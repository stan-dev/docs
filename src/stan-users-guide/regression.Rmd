...
R \times \frac{1}{\sqrt{n - 1}}$. Thus, we can equivalently write $\eta = x
...
$$
\operatorname{logit}^{-1}(u) = \mathtt{inv\_logit}(u) = \frac{1}{1 + \exp(-u)}.
$$

The model formulation above uses the logit-parameterized version of
the Bernoulli distribution, which is defined by
$$
\mathtt{bernoulli\_logit}\left(y \mid \alpha \right)
=
\mathtt{bernoulli}\left(y \mid \operatorname{logit}^{-1}(\alpha)\right).
$$
...
$$
\Phi(x) = \int_{-\infty}^x \mathsf{normal}\left(y \mid 0,1 \right) \,\textrm{d}y.
$$
...
to a simplex,
$$
\mathtt{categorical\_logit}\left(y \mid \alpha\right)
=
\mathtt{categorical}\left(y \mid \mathtt{softmax}(\alpha)\right),
$$
where
$$
\mathtt{softmax}(u) = \exp(u) / \operatorname{sum}\left(\exp(u)\right).
$$
...
Bernoulli,^[The Bernoulli-logit distribution builds in the log link function, taking $$\mathtt{bernoulli\_logit}\left(y \mid \alpha\right) = \mathtt{bernoulli}\left(y \mid \operatorname{logit}^{-1}(\alpha)\right).$$]
...
$$
\mathsf{Pr}[y_n = 1] = \operatorname{logit}^{-1}\left(\alpha_{jj[n]} - \beta_{kk[n]}
+ \delta\right).
$$
...
$$
\mathtt{std\_normal}(y)
=
\mathsf{normal}\left(y \mid 0, 1\right).
$$
...
$$
\Sigma = \mathtt{diag\_matrix}(\tau) \times \Omega \times \mathtt{diag\_matrix}(\tau),
$$
...
$$
\mathsf{LKJCorr}\left(\Sigma \mid \eta\right)
\propto
...
\propto
\prod_{n=1}^N
  \mathsf{normal}\left(y_n \mid \alpha + \beta x_n, \sigma\right).
$$
...
= \int_{(\alpha,\beta,\sigma)}
  \mathsf{normal}\left(\tilde{y}_n \mid \alpha + \beta \tilde{x}_n, \sigma\right)
  \times
  p\left(\alpha, \beta, \sigma \mid x, y\right)
