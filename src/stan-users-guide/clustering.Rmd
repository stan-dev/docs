...
$$
z_n \sim \textsf{categorical}(\textbf{1}/K),
$$
where $\textbf{1}$ is the unit vector of $K$ dimensions, so that $\textbf{1}/K$
...
$$
\Sigma_k = \texttt{diag}\mathtt{\_}\texttt{matrix}(\textbf{1}),
$$
so that the log multivariate normal can be implemented directly up to a proportion
by
$$
\textsf{normal}\left( y_n \mid \mu_k, \texttt{diag\_matrix}(\textbf{1}) \right)
...
$$
\textsf{Pr}[z_m = k \mid w,\alpha,\beta]
=
\exp \left(
\gamma_{m,k}
...
$$
\textsf{LKJCorr}(\Omega \mid \alpha) \propto \operatorname{det}(\Omega)^{\alpha - 1}
$$
