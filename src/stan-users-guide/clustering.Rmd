...
$$
where $\mathbf{1}$ is the unit vector of $K$ dimensions, so that $\mathbf{1}/K$
...
$$
\Sigma_k = \mathtt{diag\_matrix}(\mathbf{1}),
$$
so that the log multivariate normal can be implemented directly up to a proportion
by
$$
\mathsf{normal}\left( y_n \mid \mu_k, \mathtt{diag\_matrix}(\mathbf{1}) \right)
...
<table>
  <thead>
    <tr><th>`n`</th><th>`w[n]`</th><th>`doc[n]`</th></tr>
  </thead>
  <tbody style="text-align: center;">
    <tr><td>1</td><td>\(w_{1,1}\)</td><td>1</td></tr>
    <tr><td>2</td><td>\(w_{1,2}\)</td><td>1</td></tr>
    <tr><td>&#8942;</td><td>&#8942;</td><td>&#8942;</td></tr>
    <tr><td>\(N_1\)</td><td>\(w_{1,N[1]}\)</td><td>1</td></tr>
    <tr><td>\(N_1+1\)</td><td>\(w_{2,1}\)</td><td>2</td></tr>
    <tr><td>\(N_1+2\)</td><td>\(w_{2,2}\)</td><td>2</td></tr>
    <tr><td>&#8942;</td><td>&#8942;</td><td>&#8942;</td></tr>
    <tr><td>\(N_1+N_2\)</td><td>\(w_{2,N[2]}\)</td><td>2</td></tr>
    <tr><td>\(N_1+N_2+1\)</td><td>\(w_{3,1}\)</td><td>3</td></tr>
    <tr><td>&#8942;</td><td>&#8942;</td><td>&#8942;</td></tr>
    <tr><td>\(N=\sum_{m=1}^M N_m\)</td><td>\(w_{M,N[M]}\)</td><td>\(M\)</td></tr>
  </tbody>
</table>
```
\begin{center}
\begin{tabular}{r|cc}
\texttt{n} & \texttt{w[n]} & \texttt{doc[n]} \\ \hline
1 & $w_{1,1}$ & 1 \\
2 & $w_{1,2}$ & 1 \\
\vdots & \vdots & \vdots \\
$N_1$ & $w_{1,N[1]}$ & 1 \\
...
\log\, &p(w_{m,1},\dotsc,w_{m,N_m} \mid \theta,\phi) \\
 &= \log \sum_{k=1}^K
    \left( \mathsf{categorical}(k \mid \theta)
           \times \prod_{n=1}^{N_m} \mathsf{categorical}(w_{m,n} \mid \phi_k)
    \right) \\
 &= \log \sum_{k=1}^K \exp \left(
    \log \mathsf{categorical}(k \mid \theta)
     + \sum_{n=1}^{N_m} \log \mathsf{categorical}(w_{m,n} \mid \phi_k)
    \right).
\end{align*}
...
$$
\gamma_{m,k} = \log \mathsf{categorical}(k \mid \theta)
+ \sum_{n=1}^{N_m} \log \mathsf{categorical}(w_{m,n} \mid \phi_k).
$$
...
$$
\mathsf{Pr}[z_m = k \mid w,\alpha,\beta]
=
\exp \left(
\gamma_{m,k}
...
\log\, &p(\theta,\phi \mid w,\alpha,\beta) \\
 &= \sum_{m=1}^M \log \mathsf{Dirichlet}(\theta_m \mid \alpha)
    + \sum_{k=1}^K \log \mathsf{Dirichlet}(\phi_k \mid \beta) \\
 &\qquad + \sum_{m=1}^M \sum_{n=1}^{N[m]} \log \left(
    \sum_{z=1}^K \mathsf{categorical}(z \mid \theta_m)
    \times \mathsf{categorical}(w_{m,n} \mid \phi_z)
  \right)
\end{align*}
...
$$
\mathsf{LKJCorr}(\Omega \mid \alpha) \propto \operatorname{det}(\Omega)^{\alpha - 1}
$$
