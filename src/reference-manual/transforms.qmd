...
Vectors that are constrained to sum to zero are useful
in regression tasks where nonidentifiability is an issue
or for ability or difficulty parameters (but not both) in an IRT model.

In constraint terms, a zero sum $K$-vector $x \in \mathbb{R}^K$ satisfies
...
TODO: Document isometric log ratio transform
...
See also, description of the same using "pivot coordinates" in
...
TODO: Document inverse isometric log ratio transform
