...
Stan samples from the unconstrained space prior to transformations.
A unit vector, as discussed below, does not have a unique
one-to-one mapping. However, to generate a random unit-vector
@Marsaglia:1972 showed that if we draw $y_n \sim \mathsf{Normal}(0, 1)$
...
The program adjusts the target posterior so that the original
unconstrained values are transformed to those from a standard
multivariate normal with $n$ independent dimensions. The adjustment
is on the log scale and is $-0.5 y^\top y$. 
...
### No absolute Jacobian determinant of the unit vector inverse transform {-}
