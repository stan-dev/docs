...
### Zero-sum matrices
These functions perform the transform and inverse transform described in the
[Zero Sum Matrix section](https://mc-stan.org/docs/reference-manual/transforms.html#zero-sum-matrix-transform).
...
The returned matrix will have one extra row and column compared to the input `y`.
...
Takes a matrix `x` with rows and columns that sum to `0` and returns the
corresponding free matrix.

This returned matrix will have one fewer rows and one fewer columns compared to the input `x`.
{{< since 2.37 >}}
