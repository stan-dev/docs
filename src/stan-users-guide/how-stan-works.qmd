...
primitive types are `int`, `real`, and `complex`.  Integer types may
not be used as parameters.  The primitive container types are
`vector`, `row_vector`, and `matrix`.  Array types are available to
hold any single type of value.  Tuples hold a sequence of values
of fixed types.
...
on variables from the previous blocks. Statements in this block may
increment the log Jacobian for changes of variables.  Executed every
log density and gradient evaluation.

6.  `model`: Define the target log density using the variables from
previous blocks.  Executed every log density and gradient evaluation.
...
quantities that are not needed for the log density evaluation.
Statements in this block may use random number generators.  Executed
once per iteration using primitives.  Execution with primitives means
no automatic differentiation and three or more times faster than
similar code in the model or transformed parameter blocks.
...
this point, an object can be constructed from data.  This object can
be used to evaluate log densities and derivatives as well as
unconstraining transforms and their inverses.

## An example Stan program

Consider the following very simple Stan program, which can be used to
estimate a proportion $\theta \in (0, 1)$ from binary observations
$y_n \in \{ 0, 1 \}$.
...
incrementing the target log density. The model block of the previous
program could be equivalently written as follows.
...
target log density function $\log p$ up to a constant,
$$
\log p(\theta \mid y, N)
...
more efficient evaluation (e.g., $\log \theta$ is only computed once
in this example).
...
predict what the next $\tilde{N}$ trials might look like.  This is
called posterior predictive inference, and in Stan, we can define the
quantities of interest in the `generated quantity` block.  We extned
our example program by declaring a data variable for the number of new
observations $\tilde{N}$ and then defining our predictive quantity in
...
data formatted as lists in R or dictionaries in Python.  For the
running example, consider the data defined by the following JSON.
...
densities and gradients. For example, we could provide a value such as
$\theta^\textrm{unc} = -1.3786352$ and the Stan program will return
the posterior log density up to a normalizing constant, $\log p(\theta
\mid y, N) + \textrm{const}$, and its gradient, $$ \nabla \left( \log
p(\theta^\textrm{unc} \mid y, N) + \textrm{const} \right) =
\frac{\partial}{\partial \theta^\textrm{unc}} \log
p(\theta^\textrm{unc} \mid y, N), $$ where the constant drops out
because it does not involve $\theta$.
...
distribituion, the Markov chain from which they arose may be
...
Stan's inference engines will also genreate all generated quantities
...
convergence, and effective sample size, which estimates the
informativeness of the potentially autocorrelated Markov chain Monte
Carlo sample in units of independent draws.  Finally, summaries report
standard error on the mean estimates, which are calculated as
standard deviation divided by the square root of the effective sample size.
...
appropriate scale after fitting on the unconstrained scale.  This
needs to be done before averaging for estimating expectations because
...
## Technical detail: Models define unconstrained log density functions

This section describe how Stan works under the hood by transforming
constrained log density functions.  Understranding the details of this
section is not necessary to write Stan code.
...
specifies all of the constrained parameters and corresponding
transforms and change-of-variables adjustments.  
...
This unconstrained log density is what Stan is actually sampling or
approximating with variational inference or Laplace approximations.
After inference, parameter values are automatically transformed back
