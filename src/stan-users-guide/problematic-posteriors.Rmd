...
$$
y_{i, j} \sim \mathsf{Bernoulli}(\operatorname{logit}^{-1}(\alpha_j - \beta_i)).
$$
...
$$
y_{i, j} \sim \mathsf{Bernoulli}(\operatorname{logit}^{-1}(\delta_i(\alpha_j - \beta_i))).
$$
...
to a $K$-simplex $\theta = \mathtt{softmax}(\alpha)$ by defining
$$
...
The many-to-one nature of $\mathtt{softmax}(\alpha)$ is typically
...
$\theta = \mathtt{softmax}(\alpha)$ in terms of an unconstrained
...
= \prod_{n=1}^N \big( \theta \, \mathsf{normal}(y_n \mid \mu_1,\sigma)
                       + (1 - \theta) \, \mathsf{normal}(y_n \mid \mu_2,\sigma) \big).
$$
...
estimate $\mathsf{Pr}[\mu_1 > \mu_2]$, will fail, because the estimator
$$
\mathsf{Pr}[\mu_1 > \mu_2]
\approx
\sum_{m=1}^M \mathrm{I}\left(\mu_1^{(m)} > \mu_2^{(m)}\right)
$$
...
$\mathsf{beta}(\phi \mid 0.5,0.5)$, which can arise if seemingly weak beta
...
p(\phi \mid y)
 &\propto 
   \mathsf{beta}(\phi \mid 0.5,0.5) \times \prod_{n=1}^N \mathsf{Bernoulli}(y_n \mid \phi) \\
 &=
   \mathsf{beta}\left(\phi \,\middle|\, 0.5 + \sum_{n=1}^N y_n, 0.5 + N - \sum_{n=1}^N y_n\right).
\end{align*}

If $N = 9$ and each $y_n = 1$, the posterior is
$\mathsf{beta}(\phi \mid 9.5,0,5)$.  This posterior is unbounded as $\phi
...
$$
y_n \sim \mathsf{Bernoulli}(\operatorname{logit}^{-1}(x_n \beta)).
$$
...
$[0,1]$ and is given a flat prior $\mathsf{uniform}(\psi \mid 0,1)$. Now if
the data don't tell us anything about $\psi$, the posterior is also
$\mathsf{uniform}(\psi \mid 0,1)$.
