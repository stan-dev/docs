...
Stan provides three different built in methods for solving systems of
ordinary differential equations (ODEs). If care is not taken, ODE models
can be difficult to fit because

- It is easy to make a mistake specifying the system of ODEs

- A misspecified ODE can be very slow to solve -- making it difficult
to quickly identify mistakes and interate

- The numerical behavior of ODE can change radically in different parts
of parameter space

- There are a large number of different ODE solvers to pick from (how
to identify the best settings for a problem is not trivial)

- The sensitivity equations necessary to get gradients for HMC can make
the ODE system scaling seem unusually expensive

These problems are only amplified by the reality of Bayesian workflow,
where it is usually desirable to iterate over many different ODE models.

With this in mind, it is important to work methodically with ODE models.

The functions in Stan for working with ODEs are:

- Three different adaptive stepsize solvers -- a Runge-Kutta solver,
an Adams-Moulton solver, and a BDF solver with default tolerances.

- A version of each solver with customizable tolerances and the ability
to specify a parameter so that the solver will give up on extra difficult
problems

- The ability to measure the time spent in any solve using `profile`
statements

The generic workflow tools are:

- Prior predictive checks to validate that a solver is doing the right thing

- Fake data fits to evaluate if a fit is possible under certain ideal
conditions

By sequentially developing the model, first my simulating from a prior to
make sure the system is specified correctly and secondly fitting to fake data
simulations, the hope is that it will be possible to configure a suitable
ODE solver for the problem at hand and avoid the problems above.
