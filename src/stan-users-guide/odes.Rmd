...
(in fact, the system being linear, the most efficient approach would
be to use the matrix exponential function).
...
real data `x`, and integer data `x_int` must
be expressions that only involve data or transformed data variables.
The initial state `y`, the initial time `t0`, the requested solution
times `ts`, or the parameters `theta` may involve parameters.
