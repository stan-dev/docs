...
<!-- matrix; to_matrix; (array[] vector vs); -->
\index{{\tt \bfseries to\_matrix }!{\tt (array[] vector vs): matrix}|hyperpage}

`matrix` **`to_matrix`**`(array[] vector vs)`<br>\newline
Convert a one-dimensional array of column vectors to a matrix, where the size of
the array is the number of columns of the resulting matrix and the length of the
vectors is the number of rows.
{{< since 2.40 >}}
...
<!-- complex_matrix; to_matrix; (array[] complex_vector vs); -->
\index{{\tt \bfseries to\_matrix }!{\tt (array[] complex\_vector vs): complex\_matrix}|hyperpage}

`complex_matrix` **`to_matrix`**`(array[] complex_vector vs)`<br>\newline
Convert a one-dimensional array of column vectors to a matrix, where the size of
the array is the number of columns of the resulting matrix and the length of the
vectors is the number of rows.
{{< since 2.40 >}}
...
\index{{\tt \bfseries to\_vector\_array }!{\tt (matrix m): array[] vector}|hyperpage}

`array[] vector` **`to_vector_array`**`(matrix m)`<br>\newline
Convert a matrix into a one-dimensional array of vectors. Each column
in the matrix becomes one entry in the array.
{{< since 2.40 >}}


<!-- array[] complex_vector; to_vector_array; (complex_matrix m); -->
\index{{\tt \bfseries to\_vector\_array }!{\tt (complex\_matrix m): array[] complex\_vector}|hyperpage}

`array[] complex_vector` **`to_vector_array`**`(complex_matrix m)`<br>\newline
Convert a matrix into a one-dimensional array of vectors. Each column
in the matrix becomes one entry in the array.
{{< since 2.40 >}}
...
<!-- array[] row_vector; to_row_vector_array; (matrix m); -->
\index{{\tt \bfseries to\_row\_vector\_array }!{\tt (matrix m): array[] row\_vector}|hyperpage}

`array[] row_vector` **`to_row_vector_array`**`(matrix m)`<br>\newline
Convert a matrix into a one-dimensional array of row vectors. Each row
in the matrix becomes one entry in the array.
{{< since 2.40 >}}

<!-- array[] complex_row_vector; to_row_vector_array; (complex_matrix m); -->
\index{{\tt \bfseries to\_row\_vector\_array }!{\tt (complex\_matrix m): array[] complex\_row\_vector}|hyperpage}

`array[] complex_row_vector` **`to_row_vector_array`**`(complex_matrix m)`<br>\newline
Convert a matrix into a one-dimensional array of row vectors. Each row
in the matrix becomes one entry in the array.
{{< since 2.40 >}}
