...
It is simplest to start with a Stan model that simulates
...
grid over some interval. For example, in R, this is: `seq(0, 100, .1)`.


### Inputs with Multiple Predictors {-}

To change from a single predictor to multiple predictors, we need to change
...
$$
where $\textbf{0}$ is an $N$-vector of 0 values and $I_n$ is the $N
\times N$ identity matrix.  Let $L$ be the Cholesky factorization of
$K(x \mid \theta)$. That is, $L$ is the lower-triangular matrix $L$ such that $LL^{\top} =
