...
It is very useful when there are large differences between the units and scales of the predictors.
...
$x = (x_1,\dotsc,x_N)$ the corresponding sequence of $N$ predictors.  A
...
$$
sd_x = {\left({\frac{1}{N} \sum_{n=1}^{N} (x_n - \mu_x)^2}\right)}^{1/2}
$$

With these, we compute the $z$, the standardized predictors

$$
z_n = \frac{x_n - \mu_x}{sd_x}
$$
...
"natural" parameters, but the model is otherwise identical.
...
when `x` is an $N \times K$ matrix of predictors
and `beta` a $K$-vector of coefficients, so `x * beta` is an
$N$-vector of predictions, one for each of the $N$ data items,
and you do not have an informative prior on the location of $\beta$,
