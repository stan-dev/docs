...
This latter form of the model is coded in `naive_linear_regression.stan` in the folder [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression):


```{r echo=FALSE, comment=NA}
print_file('example_code/simple_linear_regression/naive_linear_regression.stan')
```
...
distributed noise term with scale `sigma`. Note that the folder [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression) contains a `run.R` and `run.py` that generate data and run the Stan programs from this section. 
...
In addition to being more concise, the vectorized form is much faster.^[Unlike in Python and R, which are interpreted, Stan is translated to C++ and compiled, so loops and assignment statements are fast.  Vectorized code is faster in Stan because (a) the expression tree used to compute derivatives can be simplified, leading to fewer virtual function calls, and (b) computations that would be repeated in the looping version, such as `log(sigma)` which is implicit in computing `normal(normal(alpha + beta * x[n], sigma)` in the above model, will be computed once and reused.]
...
one predictor can be written directly using matrix notation. See `vectorized_linear_regression.stan` in  [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression)

```{r echo=FALSE}
print_file('example_code/simple_linear_regression/vectorized_linear_regression.stan')
```
...
there is no longer an intercept coefficient `alpha`. See `matrix_w_intercept_linear_regression.stan` in  [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression). Instead, we have assumed that the first column of the input matrix `x` is a column of 1 values.  This way, `beta[1]` plays the role of the intercept.  If the intercept gets a different prior than the slope
...
\theta$. In that case, the previous Stan program is transformed to `QR_regression.stan` in [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression): 
```{r echo=FALSE}
print_file('example_code/simple_linear_regression/QR_regression.stan')
```
...
informative prior that scales with the outcome will do. A centered/scaled version is `centered_scaled_linear_regression.stan` in [simple_linear_regression/](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_linear_regression).
...
sampling distribution is changed as shown below with complete code in `student_t_regression.stan` in the folder [student_t_regression/]( https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/student_t_regression).
...
A logistic regression model with one predictor and an intercept is coded in `logistic_regression.stan` in the folder  [simple_logistic_regression](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_logistic_regression): 
```{r echo=FALSE}
print_file('example_code/simple_logistic_regression/logistic_regression.stan')
```
...
```
The example file `probit_regression.stan` is in the folder  [simple_probit_regression](https://github.com/stan-dev/docs/tree/footnote/src/stan-users-guide/example_code/simple_probit_regression).
...
$$
\beta_j \sim \textsf{multivariate normal}(\mu, \, \Sigma)
