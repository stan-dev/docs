...
$\operatorname{logit}(u)$ is distributed as $\mathsf{logistic}(0, 1)$.  This
...
$$
p(y)
= \mathsf{normal}(\log y \mid \mu, \sigma) \, \left| \frac{d}{dy} \log y \right|
= \mathsf{normal}(\log y \mid \mu, \sigma) \, \frac{1}{y}.
$$
Stan works on the log scale to prevent underflow, where
$$
\log p(y)
=
\log \mathsf{normal}(\log y \mid \mu, \sigma) - \log y.
$$
...
Jacobian required, because the transform from $(L, \alpha_{\mathrm{raw}})$
to $(L + \alpha_{\mathrm{raw}}, \alpha_{\mathrm{raw}})$ produces
