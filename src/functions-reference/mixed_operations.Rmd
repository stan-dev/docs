...
<!-- complex_matrix; to_matrix; (complex_matrix M, int m, int n, int col_major); -->
\index{{\tt \bfseries to\_matrix }!{\tt (complex\_matrix M, int m, int n, int col\_major): complex\_matrix}|hyperpage}

`complex_matrix` **`to_matrix`**`(complex_matrix M, int m, int n, int col_major)`<br>\newline
Convert a matrix M to a matrix with m rows and n columns filled in
row-major order if col_major equals 0 (otherwise, they get filled in
column-major order).
`r since("2.30")`
...
Convert the row vectorv to a column vector.
`r since("2.30")`
...
Convert the one-dimensional complex array to a column vector.
`r since("2.30")`
