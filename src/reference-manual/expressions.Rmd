...
only promotion allowed in Stan is integer -> real -> complex; if one
argument is of type `int` and the other of type `real`, the
conditional expression as a whole is of type `real`.  In
...
The second promotion rule is that expressions of type `real` may be
used anywhere an expression of type `complex` is required.  A real
value is promoted to a complex number with that real component and a
zero imaginary component.

Promotion is transitive, so that integers may be promoted to complex
...
used where `array [,] complex` is required.
...
arguments whos types match those at the end of `foo` and returns an array of
vectors.
