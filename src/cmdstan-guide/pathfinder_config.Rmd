...
The Pathfinder algorithm is described in section [Pathfinder overview](#pathfinder_intro).
...
The Pathfinder algorithm uses arguments `init_alpha` through `history_size` to run the L-BFGS optimizer.
The remaining arguments are specific to the Pathfinder algorithm.
...
These arguments have the same defaults as for [optimization](#optimize_config).
...
The importance resampled draws are output as a [StanCSV file](#pathfinder_csv) file.
...
## Individual Pathfinder Outputs

The boolean option `save_single_paths` is used to save the outputs from the individual Pathfinder runs.
By default, the single pathfinder outputs are not saved.
When `save_single_paths` is $1$, the draws from each are saved to
StanCSV files with the same format as the PSIS sample
a record of the ELBO evaluations along the L-BFGS trajectory is saved as JSON.
...
Option `num_paths=1` runs just one single Pathfinder, in which case the
output CSV file contains just the sample from that run.
The combination of arguments `num_paths=1 save_single_paths=1` results int two output files,
the CSV sample and the JSON file of ELBO iteration information.
In this case, no distinguishing suffixes are needed.
