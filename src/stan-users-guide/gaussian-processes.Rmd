...
$$
y \sim \mathsf{multivariate\ normal}(m(x), K(x \mid \theta)),
$$
...
return the zero vector, $m(x) = \mathbf{0}$.  Consider the following
...
$$
where $\mathbf{0}$ is an $N$-vector of 0 values and $\mathbf{1}$ is the $N
...
target distribution,
$$
  \mu + L\eta \sim \mathsf{multivariate\ normal}(\mu(x), K(x \mid \theta)).
$$
...
f      &\sim \mathsf{multivariate\ normal}\left(0, K(x \mid \alpha, \rho)\right) \\
...
y      &\sim \mathsf{multivariate\ normal}
         \left(0, K(x \mid \alpha, \rho) + \mathbf{I}_N \sigma^2\right) \\
\end{align*}
...
$$
z_n \sim \mathsf{Bernoulli}(\operatorname{logit}^{-1}(y_n)),
$$
or in other words,
$$
\mathsf{Pr}[z_n = 1] = \operatorname{logit}^{-1}(y_n).
$$
...
GP, i.e.:
\begin{align*}
f   &\sim \mathsf{multivariate\ normal}\big(0, K(x \mid \alpha, \rho)\big) \\
y_i &\sim \mathsf{normal}\left(\beta_0 + x_i \beta_{[1:D]} + f_i, \sigma\right) \, \forall i
  \in \{1, \dots, N\} \\
...
=
\mathsf{normal}\left(K^{\top}\Sigma^{-1}y,\
...
\begin{align*}
y_i  &\sim \mathsf{multivariate\ normal}\left(f(x_i), \mathbf{I}_M \sigma^2\right) \\
f(x) &\sim \mathsf{GP}\big(m(x), K(x \mid \theta, \phi)\big) \\
...
\begin{align*}
f        &\sim \mathsf{matrixnormal}\big(m(x), K(x \mid \alpha, \rho), C(\phi)\big) \\
...
$$
f_{[n,]} \sim \mathsf{multivariate\ normal}\big(m(x)_{[n,]}, K(x \mid \alpha,
\rho)_{[n,n]} C(\phi)\big)
$$
and that the columns of the matrix $f$ are
distributed:
$$
f_{[,m]} \sim \mathsf{multivariate\ normal}\big(m(x)_{[,m]}, K(x
...
f          &\sim \mathsf{matrixnormal}\big(0, K(x \mid 1.0, \rho), C(\phi)\big) \\
\eta       &\in \mathbb{R}^{N \times M} \\
L_C(\phi)  &= \mathtt{cholesky\_decompose}\big(C(\phi)\big) \\
L_{K(x \mid 1.0, \rho)} &= \mathtt{cholesky\_decompose}\big(K(x \mid 1.0, \rho)\big)
\end{align*}
