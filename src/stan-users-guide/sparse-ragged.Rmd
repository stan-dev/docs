...
with missing responses shown as NA, as in R.  There is no support within
Stan for R's NA values, so this data structure cannot be used
directly.
Instead, it must be converted to a "long form" as in a database,
with columns indicating the indeces along with the value.
...
kable(df, booktabs=TRUE, escape=FALSE, linesep="") %>%
  kable_styling(full_width=FALSE) %>% add_header_above()
```

With columns $jj$ and $kk$ used for the indexes
(following @GelmanHill:2007), the 2-D array $y$ (above)
can be recoded as a table (below).
