...
$$
\frac{1}{M} \sum_{i = 1}^M \textrm{Pr}(Z = k \mid \Theta = \theta^{(i)}).
$$
...
$$
\frac{1}{M} \sum_{i = 1}^M \textrm{Pr}(Z = k \mid \Theta = \theta^{(i)}, Y).
$$
...
$$    
\textrm{Pr}(Z = k \mid \Theta = \theta^{(i)}, Y) 
= \frac{\textrm{Pr}(Y \mid Z = k, \Theta = \theta^{(i)}) 
\textrm{Pr}(Z = k \mid \Theta = \theta^{(i)})}
{\sum_{k = 1}^K \textrm{Pr}(Y \mid Z = k, \Theta = \theta^{(i)}) 
\textrm{Pr}(Z = k \mid \Theta = \theta^{(i)})}.
$$
Here, $\textrm{Pr}(Y \mid \Theta = \theta^{(i)}, Z = k)$ is the likelihood conditional on a particular value of the latent variables. Crucially, all elements of the expression can be calculated using the samples from the posterior of the continuous parameters and knowledge of the model structure. Note that for certain models, such as zero-inflated count models [Section 5.6], some values of the latent parameters and data may by incompatible, giving exactly zero probabilities.

Other than the use of Bayes's theorem, $\textrm{Pr}(Z = k \mid \theta = \theta^{(i)}, Y)$ can also be extracted by coding the Stan model to include the conditional probability explicitly (as is done for the Dawid--Skene model in [Section 7.4]).
