...
For constructing vectors and matrices in Stan, see [Vector, Matrix, and Array Expressions](#vector-matrix-array-expressions.section).
...
#### Array syntax before Stan 2.26

Before Stan 2.26, arrays were declared by enclosing the dimensions
in square brackets following the name of the variable.
...
Both syntaxes can be used, though the `array` keyword syntax is recommended.
...
declares a variable `j` that is an integer. 
...
This can be combined with assignments whenever a declare-define is valid, as 
documented in
[the previous section](#compound-variable-declaration-and-definition):
