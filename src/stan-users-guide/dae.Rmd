...
(DAEs). The solver adaptively
refine their solutions in order to satisfy given tolerances.
...
kinetics problem[@robertson_solution:1966]. The normalized DAE consists of two differential equations
...
The state equations implicitly defines the state $(x(t), y(t), z(t))$ at future times
...
explicitly depends on `t`, however one still needs to specify `t` as
an argument.
...
## Control parameters for DAE solving {#control-ode.section}
