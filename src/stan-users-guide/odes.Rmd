...
Because Stan requires gradient information for sampling, the ODE
...
sensitivities). Two fundamentally very different approaches are available
in Stan to solve this problem, each having very different
computationals cost depending on the number of ODE
...
The scaling of the adjoint sensitivuty problem is much better than the
forward sensitivity problem. However, the adjoint problem is harder to
configure and the overhead for small problems actually makes it slower
than solving the full forward sensitivity system. With that in mind,
the rest of this introduction focuses on the forward sensitivity
interfaces. For information on the adjoint sensitivity interface
see the [Adjoint ODE solver](#adjoint-ode.section)
...
forward ODE approach augments the original ODE system with $N$ additional
states for each parameter for which gradients are needed. If there are $M$
parameters, and ODE system with $N \cdot (M + 1)$ states must be solved
to compute the solution with sensitivities. This can result in very
large ODE systems through the multiplicative scaling of the computational
effort needed.

In contrast, the adjoint ODE solver integrates forwarded in time a
...
as the `bdf` and `adams` forward sensivitity interfaces).
