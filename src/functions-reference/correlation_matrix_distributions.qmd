---
pagetitle: Correlation Matrix Distributions
---
...
unit diagonal (i.e., a correlation matrix), then \begin{equation*}
\text{LkjCorr}(\Sigma|\eta) \propto \det \left( \Sigma \right)^{(\eta
- 1)}. \end{equation*} The expectation is the identity matrix for any positive
...
(i.e., a correlation matrix), then \begin{equation*} \text{LkjCholesky}(L|\eta)
\propto \left|J\right|\det(L L^\top)^{(\eta - 1)} = \prod_{k=2}^K
L_{kk}^{K-k+2\eta-2}. \end{equation*} See the previous section for details on
