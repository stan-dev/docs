...
a supplied index is valid (not out of range). This additional check avoids segmentation faults and other runtime errors 
...
`STAN_NO_RANGE_CHECKS=true` makefile flag in the `make/local` file the range checks can be ignored at compile time.
We advise using this flag with caution and once the indexing has been validated. In case of any unexpected behaviour
