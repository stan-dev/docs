...
The preferred style is fully bracketed, as follows.
...
Finally, single-statement blocks can also be written on a single 
...
This style can be much harder to read than the first example. Unless
...
brackets, but should be bracketed in general. The use of loops and 
conditionals without brackets can be dangerous.  
...
Therefore, one should use braces in all instances.
...
allowing real or integer values to act as conditions, this
should be avoided.

The condtional expression should never be a value that is not produced
 as the result of a comparison or boolean operation. Therefore, use
...
Another exception to the above rule is when the operator is the 
assignment operator `=` used inside a type constraint, such as
