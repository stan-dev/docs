...
grows without bound.  See the [plot of Neal's funnel density](reparameterization-section.html#example-neals-funnel), which has similar behavior.
...
distributed as in the original model, where the mean is $\gamma u$ and the variance,

$$
\mathbb{E}[\beta \beta^\top] ={V}[\beta] = \mathrm{diag}(\tau)\,\Omega_L\,\mathbb{V}[z]\,(\mathrm{diag}(\tau)\,\Omega_L)' = \mathrm{diag}(\tau)\,\Omega\,\mathrm{diag}(\tau) = \Sigma,
$$
is the same. Where we have used the fact that $\mathbb{V}[z] = I$.
