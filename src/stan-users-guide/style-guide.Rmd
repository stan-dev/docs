...
The **recommended** fully explicit bracketed way is as follows.
...
These can be much harder to read than the first example. Unless
there are many similar cases, it's almost always clearer to put
each sampling statement on its own line.

Conditional and looping statements may also be written without 
brackets.
...
Therefore, we recommend using braces in all instances for clarity.
...
integer values that are not produced as the result of a
...
One should be careful using real values in this way, as direct
comparison can have unexpected results due to numerical accuracy.
...
An exception to the above rule is when the operator is the assignment 
operator `=` used inside a type constraint, such as

```
real<lower=0> x;
```

Here, the preferred style is to omit spaces, except for those following commas as in
```
...
aligning the operator to indicate scoping.  For example, use 
the following form 
(though not the content; inverting matrices is almost always a bad idea).
...
It is preferred that commas be followed by spaces in function 
arguments and other instances for clarity.
