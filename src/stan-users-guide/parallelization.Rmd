...
the observed data. When in doubt which parts of the model is the most 
...
execute across heterogeneous platforms. An OpenCL program can be run on CPUs, GPUs,
DSPs or FPGAs. In order to run OpenCL programs, an OpenCL runtime be installed on
they target system.
...
* GLM functions are used in cases where X is not a `row_vector`.
