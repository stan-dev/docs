...
## Assignment statements {#assignment-statement.section}
...
`array[,] real` and `b` is an array of type `array[] real`, then
...
vice-versa.  That is, we can assign an expression of type `int` to an
lvalue of type `real`, and we can assign an expression of type `real`
to an lvalue of type `complex`.  Furthermore, promotion is transitive,
so that we can assign an expression of type `int` to an lvalue of type
`complex`.

Promotion extends to containers, so that arrays of `int` can be
promoted to arrays of `real` during assignment, and arrays of `real`
can be assigned to an lvalue of type array of `complex`.  Similarly,
an expression of type `vector` may be assigned to an lvalue of type
...
_Stan allows compound arithmetic and assignment statements of the
forms listed in the table.  The compound form is legal whenever the
corresponding long form would be legal and it has the same effect._
...
follows.^[The current notation replaces two previous versions.  Originally, a variable `lp__` was directly exposed and manipulated;  this is no longer allowed.  The original statement syntax for `target += u` was `increment_log_prob(u)`, but this form was removed in Stan 2.33]
...
following model has a single variable according to a unit normal
probability.
...
density, and the unnormalized forms drop terms which are not directly or
indirectly a function of the model parameters.

For instance, the `normal_lpdf` function returns the log density of a normal
distribution:
...
section](#compound-arithmetic-assignment.section), it is treated as a
primitive statement because `target` is not itself a variable. So,
even though
...
In Stan, a distribution statement is a notational convenience
...
drops the normalizing constants and can equivalently be expressed 
as a direct increment on the total log unnormalized probability
density as
...
formed if and only if the corresponding log probability increment statement is
...
Although both lead to the same inference algorithm behavior in Stan,
there is one
critical difference between using the distribution statement, as in
...
The distribution statement drops all the terms in the log probability
...
to `beta` here, and then used on the left-hand side of a distribution
...
variate in the distribution statement is within the bounds of the
truncation.
...
This truncated distribution statement has the same behavior as the
following code.
...
The upper truncated distribution statement
...
adjustment.  The upper-truncated distribution statement
...
For a truncated distribution statement, if the value sampled is not within
...
Note that while a lower-and-upper truncated distribution may generate a
...
array of type `array[] real`, and `mu` and `sigma` are variables of type
...
The type of the loop variable is `int`.  Unlike in C++ and similarly
to R, this variable must not be declared explicitly.
...
Consequently, if `ys` is a two dimensional array `array[,] real`, `y` will
be a one-dimensional array of real values (type `array[] real`).  If 'ys` is
...
The condition must be an integer expression and the body can
...
## Break and continue statements {#break-continue-statements}
...
will print as `(1.2,-3.5)`, with no space after the comma or within
the parentheses.
...
vector `y` is used in the distribution statement.  By printing the
value of the log probability accumulator before and after each
distribution statement, it's possible to isolate where the log probability
...
Rejections are fatal in the transformed data block. This is because if
initialization fails there is no way to recover values, so the
algorithm will not begin execution.
...
Fatal errors in other blocks may also be signaled by use of the
`fatal_error` statement.
...
The Stan `fatal_error` statement provides a mechanism to
report errors or problematic values encountered during program
execution and uniformly halt processing.

Like the `print` or `reject` statements, the fatal error statement
accepts any number of quoted string literals or Stan expressions as arguments.

The fatal error may be used to signal an unrecoverable error in blocks
where `reject` leads to the algorithm attempting to try again,
such as the `model` block.
