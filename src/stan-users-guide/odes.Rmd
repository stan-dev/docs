...
differential equations (ODEs). All of these methods adaptively
refine their solutions in order to satisfy the given tolerances.

Each solver has a one interface with default tolerances and then another
one that allows the tolerances to be modified. Picking tolerances is important
for making any of the solvers work well -- the defaults will not work everywhere.
The tolerances themselves should be chosen primarily with consideration to the
scales of the solutions and how these solutions are used in the model. For
...
It is also often useful to make the solver absolute tolerance smaller when
components of the solution go very near zero. For information on choosing
...
is best suited to each problem. Additionally, as changes are made to an ODE
