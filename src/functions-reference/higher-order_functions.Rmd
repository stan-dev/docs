...
Similar to DAE solvers, the DAE solver function takes extra arguments
that are passed along unmodified to the user-supplied system function.
Because there can be any number of these arguments and they can be of different types,
...
<!-- array[] vector; dae; (function dae, vector initial_state, vector initial_state_derivative, real initial_time, array[] real times, ...); -->
\index{{\tt \bfseries dae }!{\tt (function dae, vector initial\_state, vector initial\_state\_derivative, real initial\_time, array[] real times, ...): array[] vector}|hyperpage}

`array[] vector` **`dae`**`(function dae, vector initial_state, vector initial_state_derivative, real initial_time, array[] real times, ...)`<br>\newline
Solves the DAE system using the backward differentiation formula (BDF)
method [@serban_user:2021].
`r since("2.29")`

<!-- array[] vector; dae_tol; (function dae, vector initial_state, vector initial_state_derivative, real initial_time, array[] real times, data real rel_tol, data real abs_tol, int max_num_steps, ...); -->
\index{{\tt \bfseries dae\_tol }!{\tt (function dae, vector initial\_state, vector initial\_state\_derivative, real initial\_time, array[] real times, data real rel\_tol, data real abs\_tol, int max\_num\_steps, ...): array[] vector}|hyperpage}

`array[] vector` **`dae_tol`**`(function dae, vector initial_state, vector initial_state_derivative, real initial_time, array[] real times, data real rel_tol, data real abs_tol, int max_num_steps, ...)`<br>\newline
...
```stan
  vector dae(real time, vector state, vector state_derivative, ...)
```

The DAE residual function should return the residuals the time and state provided. The length of the returned
vector must match the length of the state input into the function.
...
The user is responsible to ensure the *`initial_state`* and
*`initial_state_derivative`*  are consistent. That is, the residual
function becomes zero at the intial time when these two arguments are introduced.
...
*   state variables passed into the residual function, derivatives
returned by the residual function, initial state passed into the
solver, and length of each vector in the output,
