...
Two user-defined functions may have the same name if they have
...
The return types of overloaded functions do not need to be the same as a matter
of course, but it may be confusing if they differ.

Two function may **not** use the same name if their signature _only_ differs by the
return type.
...
promotion](#promotion) results in situations where there are multiple valid
...
For example, the overloaded functions
```stan
real foo(int a, real b);
real foo(real a, int b);
```
Do **not** have a unique minimum when called with two integer arguments
`foo(1,2)`, and therefore cannot be called as such.

For this purpose, promotion of integers to complex numbers is considered as two
separate promotions, one from int to real and a second from real to complex.
...
real bar(complex z);
```
do have a deterministic choice when called with an integer argument: the first
signature is used.
