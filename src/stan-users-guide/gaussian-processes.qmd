...
For computational reasons, we may want to integrate out the latent variable
...
marginalized out. Instead, we can recover the latent variables in `generated quantities`:
```stan
generated quantities {
...
```
The tuple `laplace_ops` is then passed to `laplace_marginal_tol`
and `laplace_rng_tol`.
```stan
model {
// ...
...
As with the Poisson GP, we cannot marginalize the latent variables exactly,
