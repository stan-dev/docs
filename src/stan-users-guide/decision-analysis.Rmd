...
based on decisions and compute the requied expected utilities.
...
share, public transportion, or cab.  Suppose the commmuter has been
taking various modes of transporation for the previous year and the
...
problem, this density will the a posterior predictive distibution
...
\int
  ...
  \, \textrm{d}\theta.
$$
Up until now, the notation has not been considering conditioning on
observed data.

The observed data for a year of commutes consists of choice of the
chosen commute mode $d^{\textrm{obs}}_n$ and observec dosts and times
...
$$
To undertand the notation, $d_n$, also written $d[n]$, is the mode of
...
on knowledge of the city and time may vary by day or time of the commute.
...
because of their comfort and environmental impact.
...
The generated quantities block defines a variable `util` where
`util[k]` will hold the utility derived from a random commute
...
column of the Stan's summary statistics or accessed programatically
