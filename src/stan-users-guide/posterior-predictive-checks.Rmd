...
model parameters `alpha, beta`, and `sigma.` The posterior predictive
...
replicated data sets, all of which have noticeably lower variance than
...
by means of a p-value, which here is just the probability the
...
\, \textrm{d}{y^{\textrm{rep}}}.
$$
Because these values should be uniformly distributed if inference is
well calibrated, values of posterior p-values very close to zero or
...
```{r include = TRUE, echo = FALSE, fig.align = "center", fig.cap = "Scatterplot of standard deviations of replicated data sets; the vertical red line is at standard deviation of original data."}
