...
Stan program for the AR(1) model is as follows.^[The intercept in this model is $\alpha / (1 - \beta)$.  An alternative parameterization in terms of an intercept $\gamma$ suggested Mark Scheuerell on GitHub is $y_n \sim \textsf{normal}\left(\alpha + \gamma \times (y_{n-1} - \alpha), \sigma\right)$.]
...
$$
h_t \sim \textsf{normal}(\mu + \phi(h_t - \mu), \sigma).
$$
...
categorical state variables $z_t \in \{ 1, \dotsc, K \}$.  These
