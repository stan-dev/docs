...
\mathbb{R}^{K \times K}$ is lower triangular and such that $L_SL_S^{\top}$
is positive definite,then for
$L_W \in \mathbb{R}^{K \times K}$ is lower triangular and such that $L_WL_W^{\top}$
is positive definite, \[
\text{WishartCholesky}(L_W|\nu,L_S) =
\text{Wishart}(W|\mu,LL^{\top}). \] If $L$ such that $L \in {L_S, L_W}$  is lower triangular and
$L^L^{top}$ is a $K \times K$ positive definite matrix, then $L_{k,k}$
...
\index{{\tt \bfseries wishart\_cholesky\_lpdf }!{\tt (matrix L_W \textbar\ real nu, matrix L_S): real}|hyperpage}
...
`real` **`wishart_lupdf`**`(matrix L_W | real nu, matrix L_S)`<br>\newline
The log of the Wishart density for lower-triangular Cholesky factor of L_W
given degrees of freedom nu and lower-triangular Cholesky factor of the
...
\index{{\tt \bfseries wishart\_rng }!{\tt (real nu, matrix L_S): matrix}|hyperpage}
