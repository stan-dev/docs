...
<!-- vector[]; ode_ckrk_tol; (function ode, vector initial_state, real initial_time, real[] times, real rel_tol, real abs_tol, int max_num_steps, ...); -->
\index{{\tt \bfseries ode\_ckrk\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, real rel\_tol, real abs\_tol, int max\_num\_steps, ...): vector[]}|hyperpage}

`vector[]` **`ode_ckrk_tol`**`(function ode, vector initial_state, real initial_time, real[] times, real rel_tol, real abs_tol, int max_num_steps, ...)`<br>\newline
Solves the ODE system for the times provided using the Cash-Karp
...
<!-- vector[]; ode_adjoint_tol_ctl; (function ode, vector initial_state, real initial_time, real[] times, real rel_tol_forward, vector abs_tol_forward, real rel_tol_backward, vector abs_tol_backward, real rel_tol_quadrature, real abs_tol_qudrature, int max_num_steps, int num_steps_between_checkpoints, int interpolation_polynomial, int solver_forward, int solver_backward,...); -->
\index{{\tt \bfseries ode\_bdf\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, data real rel\_tol\_forward, data vector abs\_tol\_forward, data real rel\_tol\_backward, data vector abs\_tol\_backward, data real rel\_tol\_quadrature, data real abs\_tol\_quadrature, data int max\_num\_steps, data int num\_steps\_between\_checkpoints, data int interpolation\_polynomial, data int solver\_forward, data int solver\_backward, ...): vector[]}|hyperpage}

`vector[]` **`ode_adjoint_tol_ctl`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol_forward, data vector abs_tol_forward, data real rel_tol_backward, data vector abs_tol_backward, data int max_num_steps, data int num_steps_between_checkpoints, data int interpolation_polynomial, data int solver_forward, data int solver_backward, ...)`<br>\newline
...
The arguments to the adjoint ODE solver are the same for specification
of the basic ODE problem. The numerical tuning parameters provided
include a greater set as the adjoint ODE solver is numerically more
complex. The adjoint ODE solver runs a forward ODE solve followed by a
backward ODE solve. During the backward ODE solve, the forward
solution is approximated by a polynomial with support points of
controllable density. Along with the backward ODE solve one quadrature
problem per parameter is solved. The different phases can be
controlled with the arguments:

*    `data` *`rel_tol_forward`*: Relative tolerance for forward solve
*    `data` *`abs_tol_forward`*: Absolute tolerance vector for each state for forward solve
*    `data` *`rel_tol_backward`*: Relative tolerance for backward solve 
*    `data` *`abs_tol_backward`*: Absolute tolerance vector for each
     state for backward solve 
...
*    `data` *`max_num_steps`*: Maximum number of time-steps to take in integrating the ODE solution between output time points for forward and backward solve
*    `data` *`num_steps_between_checkpoints`*: number of steps between checkpointing forward solution
*    `data` *`interpolation_polynomial`*: can be 1 for hermite or 2 for polynomial interpolation method of CVODES  
*    `data` *`solver_forward`*: solver used for forward ODE problem:
     1=Adams (non-stiff), 2=BDF (stiff)
*    `data` *`solver_backward`*: solver used for backward ODE problem: 1=Adams (non-stiff), 2=BDF (stiff)
