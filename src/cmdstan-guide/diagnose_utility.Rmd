...
`bin/diagnose`, it outputs a message that all transitions are
...
To fully exercise the `diagnose` command, we take the Neal's funnel model,
...
The model is run with four chains, creating output files `output_1.csv`, ..., `output_4.csv`.
The `diagnose` command is then run as follows:
```
...
a {\it validity} concern, hitting the maximum treedepth is an {\it
efficiency} concern. Configuring the No-U-Turn-Sampler (the variant of
HMC used by Stan) involves putting a cap on the depth of the trees
...
usually due to heavy-tailed posteriors and can sometime be remedied by
