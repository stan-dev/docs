...
transformed data {
  ...
  laplace_ops.2 = 500;               // maximum number of steps for optimizer.
  laplace_ops.4 = 1;              // hessian block size, assuming the prior
                                  // covariance $K$ is block-diagonal.
  laplace_ops.5 = 1;              // solver type being used.
  laplace_ops.6 = 1000;           // max number of steps for linesearch.
  laplace_ops.7 = 1;              // allow_fallback (1: TRUE, 0: FALSE)
```
...
// ...
  ...
  target += laplace_marginal_tol(ll_function, (a, y),
                                 cov_function, (rho, alpha, x, N, delta),
                                 laplace_ops);
}
