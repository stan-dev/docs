...
- `total_time`, The combined time spent executing statements inside the `profile` which includes calculation with and without auto differentiation.
- `forward_time`, The time spent executing forward passes of reverse mode auto differentiation and evaluating statements without auto differentiation.
- `reverse_time`, The time spent executing reverse (backward) passes of reverse mode auto differentiation.
- `chain_stack`, The number of objects allocated on the chaining auto differentiation stack. There is a function call for each of these objects in
the reverse pass.
- `no_chain_stack`, The number of objects allocated on the non-chaining auto differentiation stack.
- `autodiff_calls`, The total number of times the `profile` statement was executed with auto differentiation.
- `no_autodiff_calls` - The total number of times the `profile` statement was executed without auto differentiation.
