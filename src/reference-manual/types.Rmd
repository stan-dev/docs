...
dimensions defined after the variable identifier.  The old syntax has
been deprecated and is described in [the deprecated functions
...
The old array syntax has been deprecated and the `array` keyword
...
A complex number is constructed with the `to_complex` function as follows.

```stan
real re = 2.0;   // real component
real im = -1.3;  // imaginary component
complex z = to_complex(re, im);
```

The real and imaginary components can be extracted using their
respective getter functions in the following way.

```stan
complex z = to_complex(2.0, -1.3);
...
number's real component, with the imgaginary component set to zero.
...
Function arguments work like assignment, meaning that expressions of
type `int` or `real` may be passed to a function as an argument of
type `complex`.
