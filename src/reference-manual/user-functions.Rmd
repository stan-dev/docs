...
or `_lupmf` functions used in the definition of `foo_lpdf`, then
there will be no difference between a `foo_lpdf` or `foo_lupdf` call.

If there are `_lupdf` and `_lupmf` functions used inside `foo_lpdf`,
then using the `_lpdf` version of the function will force the internal
distributions to normalize:
