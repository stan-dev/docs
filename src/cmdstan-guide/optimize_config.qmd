...
If there are constrained parameters, Stan makes [a transformation to an
unconstrained space](https://mc-stan.org/docs/reference-manual/transforms.html) and runs the optimization algorithm in the unconstrained space.
...
If the Jacobian adjustment is not included (the default), the
optimization returns parameter values that correspond to a mode of the
target in the constrained space (if such mode exists). Thus this
option is useful for any optimization where we want to find the mode
in the original constrained parameter space.

If the Jacobian adjustment is included, the optimization returns
...
[Laplace sampling](laplace_sample_config.qmd), as then Jacobian
adjustment needs to be included for correct results.
