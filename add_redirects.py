...
def main():
    ...
    for file in files:
        ...
        # WSL # 
        filename = "/".join([no_version_dir, file.replace("functions-reference\\","").replace("reference-manual\\","").replace("stan-users-guide\\","")])
        #filename = "/".join([no_version_dir, file])
        print(filename)
        # WSL # 
        r_to = "/".join([stan_site,version_dir, file.replace("functions-reference\\","").replace("reference-manual\\","").replace("stan-users-guide\\","")])
