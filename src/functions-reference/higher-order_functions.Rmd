...
cat(' * <a href="old-functions-ode-solver.html">Old integrate_ode_X ODE Interface</a>\n')
...
Solving an initial value ODE means given a set of differential equations $y' = f(y, t)$ and initial conditions $y(t_0)$, solving for $y$ at a sequence of times $t_0 < t_1 \leq t_2, \cdots \leq t_n$. $f(y, t)$ is referred to here as the ODE system function.
...
Dormand-Prince 5 algorithm in Boost.
...
Solves the ODE system for the times provided using the Runge-Kutta
Dormand-Prince 5 algorithm in Boost with additional
...
*   *`ode`*: ODE system function with signature `(real, vector, ...):vector`

The arguments represent (1) time, (2) system state, and a sequence of arguments of any type passed unmodified from the integration function call. The return value contains the derivatives with respect to time of the state.
...
*   *`...`*: arguments that will be passed through unmodified to the ODE system function. The types here must match the types in the `...` portion of the ode system function arguments.

For the versions of the ode integrator functions ending in `_tol`, these three
parameters must also be provided after `times` and before the `...` arguments:
...
### Specifying the ODE System Function

The ODE system function must have the a `vector` return type, and the first two arguments must be a `real` and `vector` in that order. These two arguments can be followed by any number of other arguments of any type:

```
 vector ode(real time, vector state, ...)
```

`...` are any number of arguments passed through unmodified from the ODE integrate
call.
...
*   *...*, all other arguments passed through from the integrate call. Each
...
*   solution times and number of vectors in the output,

## Old integrate_ode_X ODE Interface {#old-functions-ode-solver}
