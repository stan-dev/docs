---
pagetitle: Simplex Distributions
---
...
_**Warning:**_
If any of the components of $\theta$ satisfies
...
The Dirichlet probability functions are overloaded to allow
...
\index{{\tt \bfseries dirichlet\_lpdf }!{\tt (vectors theta \textbar\ vectors alpha): real}|hyperpage}

`real` **`dirichlet_lpdf`**`(vectors theta | vectors alpha)`<br>\newline
The log of the Dirichlet density for simplex(es) `theta` given prior counts
(plus one) `alpha`
`r since("2.12, vectorized in 2.21")`

<!-- real; dirichlet_lupdf; (vectors theta | vectors alpha); -->
\index{{\tt \bfseries dirichlet\_lupdf }!{\tt (vectors theta \textbar\ vectors alpha): real}|hyperpage}

`real` **`dirichlet_lupdf`**`(vectors theta | vectors alpha)`<br>\newline
The log of the Dirichlet density for simplex(es) `theta` given prior counts
