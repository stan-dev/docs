...
The easiest (but not the only) way to install a C++ development
...
to the `PATH` environment variable.  Specifically add:
...
include `BIT=32` in CmdStan `make/local` file described in the previous section.
...
The CmdStan release tarfile contains all source files an libraries needed to build CmdStan.
The [CmdStan GitHub repo](https://github.com/stan-dev/cmdstan)
contains just the `cmdstan` module;
