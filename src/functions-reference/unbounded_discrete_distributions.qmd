...
## Yule Simon distribution {#yule-simon}
...
\index{{\tt \bfseries yule\_simon\_lpmf }!{\tt (ints n \textbar\ reals alpha): real}|hyperpage}

`real` **`yule_simon_lpmf`**`(ints n | reals alpha)`<br>\newline
The log Yule Simon probability mass of `n` given parameter `alpha`.
{{< since 2.39 >}}

<!-- real; yule_simon_lupmf; (ints n | reals alpha); -->
\index{{\tt \bfseries yule\_simon\_lupmf }!{\tt (ints n \textbar\ reals alpha): real}|hyperpage}

`real` **`yule_simon_lupmf`**`(ints n | reals alpha)`<br>\newline
...
\index{{\tt \bfseries yule\_simon\_cdf }!{\tt (ints n \textbar\ reals alpha): real}|hyperpage}

`real` **`yule_simon_cdf`**`(ints n | reals alpha)`<br>\newline
The Yule Simon cumulative distribution function of `n` given parameter `alpha`.
{{< since 2.39 >}}

<!-- real; yule_simon_lcdf; (ints n | reals alpha); -->
\index{{\tt \bfseries yule\_simon\_lcdf }!{\tt (ints n \textbar\ reals alpha): real}|hyperpage}

`real` **`yule_simon_lcdf`**`(ints n | reals alpha)`<br>\newline
...
\index{{\tt \bfseries yule\_simon\_lccdf }!{\tt (ints n \textbar\ reals alpha): real}|hyperpage}

`real` **`yule_simon_lccdf`**`(ints n | reals alpha)`<br>\newline
...
\index{{\tt \bfseries yule\_simon\_rng }!{\tt (reals alpha): R}|hyperpage}

`R` **`yule_simon_rng`**`(reals alpha)`<br>\newline
Generate a Yule Simon variate with parameter `alpha`;
may only be used in transformed data and generated quantities blocks.
`alpha` $/$ (`alpha`$-1$) must be less than $2 ^ {29}$.
For a description of argument and return types, see section [vectorized function signatures](conventions_for_probability_functions.qmd#prob-vectorization).
{{< since 2.39 >}}