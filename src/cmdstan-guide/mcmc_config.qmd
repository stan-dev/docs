...
to eliminate an expected autocorrelation in the draws.
...
Thinning should only be used in circumstances where storage of the draws
is limited and/or RAM for later processing the draws is limited.
...
A Markov chain generates draws from the target distribution only after it has converged to equilibrium.
...
including mean, standard deviation, percentiles, effective sample size, and $\hat{R}$ values.
...
setting `num_warmup=0` and `num_samples=1` saves unnecessary iterations.
