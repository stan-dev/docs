...
<!-- real; quantile; (data vector, data real p); -->
\index{{\tt \bfseries quantile }!{\tt (data real[] x, data real p): real}|hyperpage}

`real` **`quantile`**`(data vector, data real p)`<br>\newline
The p-th quantile of xs.

<!-- real[]; quantile; (data vector, data real[] p); -->
\index{{\tt \bfseries quantile }!{\tt (data real[] x, data real[] p): real}|hyperpage}

`real[]` **`quantile`**`(data vector, data real ps[])`<br>\newline
An array containing the p-th quantile of xs for each p in ps.

<!-- real; quantile; (data row_vector, data real p); -->
\index{{\tt \bfseries quantile }!{\tt (data real[] x, data real p): real}|hyperpage}

`real` **`quantile`**`(data row_vector, data real p)`<br>\newline
The p-th quantile of xs.

<!-- real[]; quantile; (data row_vector, data real[] p); -->
\index{{\tt \bfseries quantile }!{\tt (data real[] x, data real[] p): real}|hyperpage}

`real[]` **`quantile`**`(data row_vector, data real ps[])`<br>\newline
An array containing the p-th quantile of xs for each p in ps.
...
\index{{\tt \bfseries linspaced\_int\_array }!{\tt (int n, int lower, int upper): real[]}|hyperpage}

`real[]` **`linspaced_int_array`**`(int n, int lower, int upper)`<br>\newline
...
\index{{\tt \bfseries svd\_U }!{\tt (matrix A): vector}|hyperpage}

`vector` **`svd_U`**`(matrix A)`<br>\newline
The left-singular vectors of A

<!-- vector; svd_V; (matrix A); -->
\index{{\tt \bfseries svd\_V }!{\tt (matrix A): vector}|hyperpage}

`vector` **`svd_V`**`(matrix A)`<br>\newline
