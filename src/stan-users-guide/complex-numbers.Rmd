# Complex Numbers {#complex.chapter}
...
constructed using the `to_complex` function as follows.

```stan
complex z = to_complex(-1.1, 2.3);  // z = -1.1 + 2.3 * i
```
...
Promotion also works for complex-valued function argumentss, which may
...
```stan
parameters {
  corr_cholesky[2] L_Omega;  // correlation matrix
...
model {
  ...
  L_Omega ~ lkj_cholesky(4);  // shrink toward diagonal corrleation
  sigma ~ ... hyperprior ...
```
...
covariance, and the prior onthe scales is left open.


### Absolute error

A third alternative for complex regression is to provide a
distribution for the magnitude of the complex error. This can be done
as follows.
...
parameters {
  ...
  ...
model {
  abs(eps_n) ~ normal(0, sigma);  // error distribution
  // log Jacobian adjustment of abs(x + yi) = sqrt(x^2 + y^2)
  target += log(fabs(get_real(eps_n) / abs_eps_n));
```

The mapping of a complex number to its absolute value can be viewed as
a two-argument function of the real and imaginary components of the
number $z = x + yi$, the absolute value of which is $\sqrt{x^2 +
y^2}$.  Because this transform is non-linear, it requires Jacobian
adjustment for the change of variables in order to induce the right
distributions on $x$ and $y$.  The transform maps a two-dimensional
input to a single value, so we can complete it to provide full rank in
a way that simplifies the determinant as
\[
f(x, y)
= \begin{bmatrix}
    \sqrt{x^2 + y^2} & y
  \end{bmatrix}$.
\]
This transform has a triangular Jacobians, so its determinant is the
product of the diagonal entries, which are
\[
J_{1, 1} = \frac{\partial}{\partial x} \sqrt{x^2 + y^2}
= \frac{x}{\textrm{abs}(x + yi)}
\]
and
\[
J_{2, 2} = \frac{\partial}{\partial y} y = 1,
\]
so that the determinant is equal to $J_{1, 1}$.  The log of the
resulting absolute value of the determinant is added to the target as
a change of variables adjustment.
