...
^[Riemannian Manifold Hamiltonian Monte Carlo (RMHMC) overcomes this difficulty by simulating the Hamiltonian dynamics in a space with a position-dependent metric; see @GirolamiCalderhead:2011 and @Betancourt:2012, but this must be done on the fly at every step, making this conditioning very computationally expensive.]
...
If vector $x$ has very large or very small values or if the
sample mean of the values is far away from 0 (on the scale of the values),
then it can be more efficient to standardize the
predictors $x_n$, so that it has a mean of zero and standard deviation of 1.
The data are first centered by subtracting the
sample mean, and then scaled by dividing by the sample deviation.
Thus a data point $u$ is standardized with respect to
a vector $x$  by the function $\textsf{z}_x$, defined by
$$
\textsf{z}_x(u) = \frac{u - \bar{x}}{\texttt{sd}(x)}
$$
where the sample mean of $x$ is
$$
\bar{x}
...
$$
\textrm{z}_x^{-1}(v) = \texttt{sd}(x) v + \bar{x}.
$$

To standardize a regression problem, standardize the predictors.
This changes the scale of the variables, and hence
changes the scale of the priors.
...
In the transformed data block the mean and standard deviation
of the data are computed and saved as global variables,
then used to define the standardized predictors.
...
$$
y_n
= \alpha' + \beta' \textrm{z}_x(x_n) + \epsilon_n.
$$
...
\qquad
\beta = \frac{\beta'}{\texttt{sd}(x)};
\end{align*}

The independent priors on alpha and beta map to a multivariate normal prior on alpha_std and beta_std. 

$$
Cov(\alpha, \beta) = 0 \implies
$$

$$
Cov(\alpha^\prime - \beta^\prime \frac{\bar{x}}{\text{sd}(x)}, \beta^\prime \frac{\bar{x}}{\text{sd}(x)}) = 0 \implies
$$

$$
Cov(\alpha^\prime,\beta^\prime) = -\frac{\bar{x}}{\text{sd}(x)} Var(\beta^\prime)
$$
...
generated quantities {
  ...
  real beta;
  real<lower=0> sigma;
  alpha = alpha_std - beta_std * mean_x / sd_x;
  beta = beta_std / sd_x;
}
```
