...
considered block variables. See the [blocks chapter](#blocks.chapter)
...
An example of a simple assignment is as follows.^[In versions of Stan before 2.18.0, the operator `<-` was used for assignment rather than using the equal sign `=`.  The old operator `<-` is now deprecated and will print a warning. In Stan 2.33, it will be removed.]
...
follows.^[The current notation replaces two previous versions.  Originally, a variable `lp__` was directly exposed and manipulated;  this is no longer allowed.  The original statement syntax for `target += u` was `increment_log_prob(u)`, but this form has been deprecated and will be removed in Stan 2.33]
