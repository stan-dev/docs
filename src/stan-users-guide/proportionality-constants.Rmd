...
the distribution does not need normalized to perform variational inference or
do optimizations).
...
The first two syntaxes drop proportionality constants:
...
The final syntax computes the full log density (dropping no constants):
...
$$
\textsf{normal_lupdf}(x | \mu, \sigma) \propto
-\frac{1}{2} \left( \frac{x - \mu}{\sigma} \right)^2
$$
...
possible to define unnormalized distributions directly (any attempt will result
in an error). However, when a custom `_lpdf` or `_lpmf` function is defined,
the compiler will automatically make available a `_lupdf` or `_lupmf` version
of the function. The difference in the normalized and unnormalized versions
of custom probability functions is that unnormalized function calls will be
replaced with their normalized equivalents if the normalized version of the
custom distribution is called.

For instance, the following code demonstrates the different behaviors:
