...
- `USER_HEADER=<header_file.hpp>`, where `<header_file.hpp>` is the name of a header file that
  defines a function with the same name and signature. This function can appear in
...
double
make_odds(const double &theta, std::ostream *pstream__)
{
...
acceptable. If you wish to autodiff through this function, make it a template,
like
...
Stan Math Library, then you would need to include
`<stan/model/model_header.hpp>` and prefix the function calls
with `stan::math::`.
