...
If our model is only a GP regression, i.e.
\begin{align*}
  y_i &= f(x_i) + \epsilon \\
  \epsilon &\sim \textsf{normal}(0, \sigma) \\
  & x \in \mathbb{R}^{N \times D}
\end{align*}

or equivalently
\begin{align*}
f   &\sim \textsf{multivariate normal}\left(0, K(x \mid \tau, l)\right) \\
y_i &\sim \textsf{normal}(f_i, \sigma) \, \forall i \in \{1, \dots, N\} \\
...
high-frequency and low-frequency functions, then our prior should put
non-negligible mass on both sets of functions.  If this is the case, an
...
small length-scales, but a diffuse right tail, allowing for large
...
prior is a prior where the limit of the prior density is zero at the boundary,
so that posterior estimates are pushed away from the boundary.].

If we're using the GP as a submodel of a larger model that includes an overall
