...
of ordinary differential equations (ODEs). Three different integrators,
...
the argument types match the types of the respective arguments in the integrate
call.
...
For additional control of the solves, both the non-stiff and stiff integrators
...
to integrate the ODE between the discontinuities, stop the solver at each
one, and restart it on the other side.

Nonetheless, the ODE integrator will attempt to integrate over discontinuities
it encounters in the state function. The accuracy of the solution near the
