...
Stan provides built-in constraint transforms for zero-sum vectors and zero-sum matrices. The zero-sum vector is a vector of length $N$ with real values and the sum of the vector equals zero. The zero-sum matrix is an $N \times M$ matrix where both the rows and columns sum-to-zero.

Stan uses an orthogonal basis as the initial point of construction. Although this may seem like an unnecessarily complicated way to construct a sum-to-zero object,the orthogonal basis construction balances the constraint across each value equally. This results in each of the constrained elements having equal variance see, e.g.,[@seyboldt:2024]. Simpler alternatives, such as setting the
final element to the negative sum of the first elements, do not result in equal variances. The $N - 1$ unconstrained parameters are independent, however, the sum-to-zero constraint induces a negative correlation across the constrained vector values.

The orthogonal basis underlying the isometric log ratio transform and the zero sum transforms in Stan is the (normalized) Helmert matrix. More details of the 
...
The mathematical expression above is equivalently expressed within Stan as follows. Although users will not have to implement this function themselves, it is sometimes easier to see the logic expressed in code. Note that there is no `target +=` expression because the Jacobian factor is 0.
...
### Zero sum vector inverse transform {-}
...
### Zero sum matrix transform {-}

The matrix case is a generalization of the vector case. In fact, any N-dimensional array can be constructed into a zero-sum N-dimensional array using the zero sum vector. This is because the vector transform is a linear bijection and produces an orthogonally constructed sum to zero object by applying the one dimensional transform across each array slice. For the matrix case, there are two slices present, the rows and columns, to perform the transform over. The sum to zero vector is applied over the vectorized slice of either the row or column slice and subsequently to the other slice. 

Let the unconstrained values of the matrix be given as
$$
...
By applying $C_n$ to each slice (or tensor mode) results in 
...
Because each $\mathbb R^{d_1\times\cdots\times d_N}$ is invertible on the $\mathbf 1^\perp$ sub-space the composite map applied to $mathcal Z$ is a linear bijection between $\mathbb R^{n_1\times n_2}$ and the codomain $\mathbb R^{(n_1+1)\times\cdots\times(n_2+1)}$.
