...
This means two different functions can be defined with the same name as long as
...
between scalar number types. Because of this, it is a programming best practice
that overloaded functions maintain the same high-level (i.e. mathematical or
semantic) meaning as each other.

### Function resolution

Stan resolves overloaded functions by the type of arguments passed to the
function.
...
promotions must be a unique minimum. This requirement does forbid certain kinds
of overloading. For example, the function signatures
```stan
...
`bar(1, 2)`. Both signatures require one promotion, and so it cannot be
...
Will select the first signature when called with an integer argument, `pop(0)`.
