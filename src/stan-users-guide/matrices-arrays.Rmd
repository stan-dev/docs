...
of type declarations in parentheses.  Tuples are constructed usingly
...
As with other container types, tuples are passed to functions by
constant reference in the underlying C++ code.
...
whereas a singlular value decomposition returns three matrices of
different shapes.  Before introducing tuples in version 2.33,
eigendecomposition of symmetric matrices involved two functions in Stan.
...
With tuples, this can be simplifed to

```stan
tuple(vector, matrix) eigendecompose_sym(matrix);
```
