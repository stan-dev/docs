...
The CmdStan method `variational` can approximate the posterior distribution using Automatic Differentiation Variational Inference (ADVI).
The approximation is a Gaussian in the unconstrained variable space.
The ADVI method uses either a fully factorized Gaussian for the approximation,
specified by argument `algorithm=meanfield` option, or a Gaussian with a
full-rank covariance matrix for the approximation, specified by argument `algorithm=fullrank`.

The executable does not need to be recompiled in order to
switch to variational inference, and the data input format is the
...
For further details, see Kucukelbir, Alp, Rajesh Ranganath, Andrew Gelman, and David M. Blei. 2015.
_Automatic Variational Inference in Stan._
arXiv 1506.03431. http://arxiv.org/abs/1506.03431.
