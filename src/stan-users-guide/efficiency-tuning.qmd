...
In some cases of exponential family likelihoods, models can be recoded
to exploit sufficient statistics in estimation.  This can lead to
large efficiency gains compared to an expanded model.  This section
provides examples for Bernoulli and normal distributions, but the same
approach can be applied to other members of the exponential family.

### Bernoulli to binomial sufficient statistics

For example, consider the following Bernoulli
sampling model.
...
This is going to evaluate the normal for `y[1]` to `y[N]`.  With the
vectorized form above, Stan is clever enough to only evaluate
`log(sigma)` once, but it still has to evaluate the normal for all of
`y[1]` to `y[N]`, which involves subtracting the mean and dividing by
the standard deviation and then squaring.

An equivalent density up to normalizing constants, is given in the
following form
...
The data and parameters are the same in this model as in the first.
This model adds a transformed data block to compute the mean and
variance of the data, which are the sufficient statistics here.  These
are stored along with two other useful constants.  Then the model
defining a distribution over data is converted to model over just the
single mean and variance value.


The original model and this one define the same log density up to a
constant additive term that does not depend on the parameters.  The
priors on `mu` and `sigma` are both improper, but they could be
upgraded to proper priors without changing the drop-in
replacement---the priors are just additional statements in the model
block.

This transform explicitly relies on aggregating the data.  If instead
this trick is used for parameters `y`, then the change of variables
adjustment for the non-linear variance calculation needs to be
included, and there won't be any efficiency gain, because the
calculations involve in variance are the same as those involved in
calculating the normal density.
