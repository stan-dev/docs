...
[ArviZ](https://python.arviz.org/) ArviZ is Python library which provides diagnostics and visualizations for Bayesian workflow.
It interfaces directly with samples from `CmdStanPy` inference methods via function [from_cmdstanpy](https://python.arviz.org/projects/base/en/latest/how_to/ConversionGuideCmdStanPy.html).
and provides the Python equivalent of the functions in the R packages `posterior`, `bayesplot`, and `loo`.
