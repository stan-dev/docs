...
declares `xi` to be a tuple holding two values, one of type `real` and
one a 5-dimensional array of type `int`.  
...
Tuples may contain nested tuples, such as

```stan
tuple(int, tuple(real, complex)) x;
```

but often this is more simply expressed as a 3-tuple of type
...
These numbers must be integer literals and must be within the size of
the number of elements of tuples.  The types of elements are as
declared, so that `abc.1` is of type `int`, `abc.2` of type
`vector[3]` and `abc.3` of type `complex`.
