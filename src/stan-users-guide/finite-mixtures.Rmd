...
=
\sum_{k=1}^K \lambda_k \, \mathsf{normal}\left(y \mid \mu_k, \sigma_k\right).
$$
...
$$
\mathtt{log\_sum\_exp}(a, b) = \log \left(\exp(a) + \exp(b)\right).
$$
...
\log\, &p\left(y \mid \lambda,\mu,\sigma \right) \\
 &= \log\big( 0.3 \times \mathsf{normal}\left(y \mid -1,2 \right)
            + 0.7 \times \mathsf{normal}\left(y \mid 3,1 \right) \big) \\
 &= \log\bigg( \exp\Big(\log\big(0.3 \times \mathsf{normal}\left(y \mid -1,2 \right)\big)\Big) 
             + \exp\Big(\log\big(0.7 \times \mathsf{normal}\left(y \mid 3,1 \right)\big)\Big) \bigg) \\
 &= \mathtt{log\_sum\_exp}\big(
                         \log(0.3) + \log \mathsf{normal}\left(y \mid -1,2 \right),
                         \log(0.7) + \log \mathsf{normal}\left(y \mid 3,1 \right) \big).
\end{align*}
...
$$
\mathtt{log\_sum\_exp}(c + a, c + b)
\ = \
c + \mathtt{log\_sum\_exp(a, b)}
$$
...
\operatorname{Pr}\left(z_n = k \mid y_n, \mu, \sigma, \lambda \right)
  ...
  &= \mathsf{normal}\left(y_n \mid \mu_k, \sigma_k\right) \cdot \lambda_k.
\end{align*}
...
\log\,&\operatorname{Pr}\left(z_n = k \mid y_n, \mu, \sigma, \lambda\right) \\
 ...
 &\quad  - \mathtt{log\_sum\_exp}_{k' = 1}^K \big(\log p\left(y_n \mid z_n = k', \mu, \sigma\right)
...
\mathsf{normal}\left(y_n \mid \mu_k,\sigma_k\right)$ is calculated and added to the
...
= \prod_{n=1}^N \big(\lambda \times \mathsf{normal}\left(y_n \mid \mu_1, \sigma_1 \right)
                 + (1 - \lambda) \times \mathsf{normal}\left(y_n \mid \mu_2, \sigma_2 \right)\big).
$$
...
$$
p\left(y \mid \lambda, \mu, \sigma \right)
= \lambda \times \prod_{n=1}^N \mathsf{normal}\left(y_n \mid \mu_1, \sigma_1\right)
+ (1 - \lambda) \times \prod_{n=1}^N \mathsf{normal}\left(y_n \mid \mu_2, \sigma_2\right).
$$
...
= \lambda \times \mathsf{normal}\left(y_n \mid \mu_1, \sigma_1\right)
+ (1 - \lambda) \times \mathsf{normal}\left(y_n \mid \mu_2, \sigma_2\right),
$$
...
=
\begin{cases}
\theta + (1 - \theta) \times \mathsf{Poisson}(0 \mid \lambda) & \quad\text{if } y_n = 0, \text{ and}\\
(1-\theta) \times \mathsf{Poisson}(y_n \mid \lambda) &\quad\text{if } y_n > 0.
\end{cases}
$$
...
(1 - \theta)
   \frac{\displaystyle \mathsf{Poisson}(y \mid \lambda)}
        {\displaystyle  1 - \mathsf{PoissonCDF}(0 \mid \lambda)}
&\quad\text{if } y > 0,
\end{cases}
$$
...
\begin{align*}
\log \left( 1 - \mathsf{PoissonCDF}(0 \mid \lambda) \right)
  &= \log \left( 1 - \mathsf{Poisson}(0 \mid \lambda) \right) \\
  &= \log(1 - \exp(-\lambda))
\end{align*}
