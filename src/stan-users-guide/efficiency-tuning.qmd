...
Stan programs will run faster if the data is standardized to have a
...
$X$ is a matrix of size $N \times J$ where $J$ is the number of predictors.
Each column $j$ of $X$ represents a feature vector $\textbf{x}_j$.

A linear regression involving an intercept coefficient $\alpha$
and an $J$-length vector of slope coefficients $\mathcal{B}$ can be expressed as
$$
y_n = \alpha + \mathcal{B} X[n, ] + \epsilon_n,
$$
where
$$
...
If any of the columns of $X$ has a mean very far from $0$
or a standard deviation far away from $1$,
then it can be more efficient to standardize the columns of $X$
so that each column has a mean of zero and standard deviation of 1.
The data are first zero-centered by subtracting the
column mean and then scaled by dividing by the column standard deviation.

The mean $\mu_j$ of column $j$ is given by:
...
The standard deviation $\sigma_j$ of column $j$ is calculated as:
$$
\sigma_j = \sqrt{\frac{1}{N} \sum_{i=1}^{N} (x_{ij} - \mu_j)^2}
$$

Each element $x_{ij}$ in column $j$ is standardized by subtracting the mean $\mu_j$ of the column and dividing by the standard deviation $\sigma_j$,
resulting in the standardized column $\mathbf{z}_j$:
...
$$
y_n
= \alpha' + \beta' \text{z}_x(x_n) + \epsilon_n.
$$
