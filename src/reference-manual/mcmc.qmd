...
$$
Here, $\log p(\rho \mid M) = \log \textrm{MultiNormal}(\rho \mid 0, M)$,
so that
$$
...
If the metric $M$ is a poor estimate of the local curvature (negative
Hessian of the target log density $\log p(\theta)$) at some
position ($\theta$) being visited on a Hamiltonian trajectory, the
...
the initial and final fast intervals and the initial size of the
...
total of $2^{10} - 1 = 1023$ leapfrog steps, with the -1 subtracting
the initial state).
