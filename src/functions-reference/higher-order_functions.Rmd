...
<!-- vector[]; ode_rk45_tol; (function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...); -->
\index{{\tt \bfseries ode\_rk45\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, data real rel\_tol, data real abs\_tol, int max\_num\_steps, ...): vector[]}|hyperpage}

`vector[]` **`ode_rk45_tol`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...)`<br>\newline
Solves the ODE system for the times provided using the Dormand-Prince
algorithm, a 4th/5th order Runge-Kutta method with additional control
parameters for the solver.

<!-- vector[]; ode_ckrk; (function ode, vector initial_state, real initial_time, real[] times, ...); -->
\index{{\tt \bfseries ode\_ckrk }!{\tt (function ode, real[] initial\_state, real initial\_time, real[] times, ...): vector[]}|hyperpage}

`vector[]` **`ode_ckrk`**`(function ode, vector initial_state, real initial_time, real[] times, ...)`<br>\newline
Solves the ODE system for the times provided using the Cash-Karp
algorithm, a 4th/5th order explicit Runge-Kutta method.

<!-- vector[]; ode_ckrk_tol; (function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...); -->
\index{{\tt \bfseries ode\_ckrk\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, data real rel\_tol, data real abs\_tol, int max\_num\_steps, ...): vector[]}|hyperpage}

`vector[]` **`ode_ckrk_tol`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...)`<br>\newline
Solves the ODE system for the times provided using the Cash-Karp
...
<!-- vector[]; ode_adams_tol; (function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...); -->
...
`vector[]` **`ode_adams_tol`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...)`<br>\newline
...
<!-- vector[]; ode_bdf_tol; (function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...); -->
\index{{\tt \bfseries ode\_bdf\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, data real rel\_tol, data real abs\_tol, int max\_num\_steps, ...): vector[]}|hyperpage}

`vector[]` **`ode_bdf_tol`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol, data real abs_tol, int max_num_steps, ...)`<br>\newline
Solves the ODE system for the times provided using the backward differentiation
formula (BDF) method with additional control parameters for the solver.

### Adjoint solver

<!-- vector[]; ode_adjoint_tol_ctl; (function ode, vector initial_state, real initial_time, real[] times, data real rel_tol_forward, data vector abs_tol_forward, data real rel_tol_backward, data vector abs_tol_backward, data real rel_tol_quadrature, data real abs_tol_qudrature, int max_num_steps, int num_steps_between_checkpoints, int interpolation_polynomial, int solver_forward, int solver_backward,...); -->
\index{{\tt \bfseries ode\_bdf\_tol }!{\tt (function ode, vector initial\_state, real initial\_time, real[] times, data real rel\_tol\_forward, data vector abs\_tol\_forward, data real rel\_tol\_backward, data vector abs\_tol\_backward, data real rel\_tol\_quadrature, data real abs\_tol\_quadrature, int max\_num\_steps, int num\_steps\_between\_checkpoints, int interpolation\_polynomial, int solver\_forward, int solver\_backward, ...): vector[]}|hyperpage}

`vector[]` **`ode_adjoint_tol_ctl`**`(function ode, vector initial_state, real initial_time, real[] times, data real rel_tol_forward, data vector abs_tol_forward, data real rel_tol_backward, data vector abs_tol_backward, int max_num_steps, int num_steps_between_checkpoints, int interpolation_polynomial, int solver_forward, int solver_backward, ...)`<br>\newline
Solves the ODE system for the times provided using the adjoint ODE solver method from CVODES. The adjoint ODE integration performs a forward and a backward ODE integration which can be controlled through additional control parameters. The parameter sensitivities are obtained as quadrature problem solved along with the backward ODE problem which can be controlled as well.
...
The arguments to the adjoint ODE solver are the same as for the
forward ODE solvers for specification of the basic ODE problem
...
The numerical tuning parameters are a large set as the adjoint ODE
solver is numerically complex. The adjoint ODE solver runs a forward
ODE solve followed by a backward ODE solve. During the backward ODE
solve, the forward solution is approximated by a polynomial with
support points of controllable density. Along with the backward ODE
solve one quadrature problem per parameter is solved. The different
phases can be controlled with the arguments:
...
*    `data` *`num_steps_between_checkpoints`*: number of steps between checkpointing forward solution, type `int`, data only,
*    `data` *`interpolation_polynomial`*: can be 1 for hermite or 2 for polynomial interpolation method of CVODES, type `int`, data only,  
*    `data` *`solver_forward`*: solver used for forward ODE problem: 1=Adams (non-stiff), 2=BDF (stiff), type `int`, data only,
*    `data` *`solver_backward`*: solver used for backward ODE problem: 1=Adams (non-stiff), 2=BDF (stiff), type `int`, data only.
