...
$$
and then simulates data given the paraemters,
$$
...
sequence of draws from the posterior
$$
\theta^{(1)}, \ldots, \theta^{(M)}
...
simulated parameter and data pairs,
$$
...
to be tested to generate $M$ posterior draws, presumbably distributed
marginally as 
$$
...
### Testing a Stan program with SBC

Now consider using simulation-based calibration to test a Stan
program.  The transformed data block is used to simulate parameters
...
The entire SBC process is as follows, where
...
correlation in the posterior draws.  This will almost always require
thinning down to the point where the effective sample size is roughly
the same as the number of thinned draws.  This may require running the
code a few times to judge the number of draws required to get a high
enough effective sample size.  This operation that can be put into a
loop that doubles the number of iterations until all parameters have
an effective sample size of `M`, then thinning down to `M` draws.
...
recommends visual inspection of the binned plots.

If there are $N$ total runs and the $J$ bins are of equal size, then
$N / J$ elements are expected to fall into each bin.
...
of ranks expected to fall into bin $j$, the test statistic is
$$
