...
This is called [_posterior predictive inference_](posterior-prediction.qmd), and in Stan, we can
...
In the same way, we can use the generated quantities block for [_posterior predictive checks_](posterior-predictive-checks.qmd)
which evaluate how well the model captures relevant aspects of the data
by generating a replicated dataset $y^{\textrm{rep}}$ of the original data set $y$.
