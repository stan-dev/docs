...
Stan provides four different methods for solving systems of ordinary
differential equations (ODEs). All of these methods will adaptively
refine their solutions in order to satisfy the given tolerances.

Each solver has a simple interface with default tolerances and then an advanced
one that allows the tolerances to be modified. If components of the solution
need to be solved with higher precision, the solution can be solved with
lower than default tolerances, or if the solution goes very near zero,
then the advanced interface may be useful. Picking tolerances is important
for making any of the solvers work well. However, the tolerances themselves
should be picked primarily with consideration to the scales of the solutions
and how these solutions are used in the model. For instance, if a solution component
slowly varies between 3.0 and 5.0 and measurements of the ODE state are noisy, then
perhaps the tolerances do not need to be as tight as for a situation where the
solutions vary between 3.0 and 3.1 and very high precision measurements of the
ODE state are available. For information on choosing tolerances, see the
[control parameters section](#control-ode.section).

The upside to adaptive solvers is that as long as reasonable tolerances are provided
and an ODE solver well-suited to the problem is chosen, the technical details
of solving the ODE can be abstracted away. The catch is that it is not always
clear from the outset what reasonable tolerances are or which ODE solver is
best suited to each problem. Additionally, as changes are made to an ODE
model, the optimal solver and tolerances may change.

With this in mind, the four solvers are `rk45`, `bdf`, `adams`, and
`ckrk`. If no other information about the ODE is available, start with the
`rk45` solver. The list below has information on when each solver is useful.

If there is any uncertainty about which solver is the best, it can be
expedient to measure the performance of all the interesting solvers
...
* `ckrk`: a fourth and fifth order explicit Runge-Kutta method for
  ...
  better for systems that exhibit rapidly varying solutions.
