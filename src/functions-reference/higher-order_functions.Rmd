...
Because the first argument to each of the solvers is a function itself,
the ODE solvers in Stan are considered high order functions.
...
the functions block. The ODE system function must have the same return
type and first two arguments as the following function:
...
*   *`...`*: arguments that will be passed through unmodified to the ODE system function, types must match those in the `...` portion of the ode system function
