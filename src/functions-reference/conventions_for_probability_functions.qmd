---
pagetitle: Conventions for Probability Functions
---
...
`normal_lpdf(y | mu, sigma)` returns the value of mathematical formula
$\log \text{Normal}(y \, | \, \mu, \sigma)$. Cumulative distribution
...
(effectively rejecting iterations or optimization steps) when they are
...
arguments, an unconstrained location $\mu$ and positive scale $\sigma > 0$,
and returns an unconstrained pseudorandom value drawn from
...
For a univariate random variable $Y$ with probability function
$p_Y(y \, | \, \theta)$, the cumulative distribution function (CDF) $F_Y$
is defined by
\begin{equation*}
...
\text{Pr}[Y > y] \ = \ 1 - F_Y(y).
\end{equation*}
The reason to use CCDFs instead of CDFs in
...
\end{equation*}
There are also log forms of the CDF and CCDF for most
