...
Computes the generalized inverse of a matrix. For a symmetric matrix `m` 
the output is equivalent to using `inverse(m)`. When `m` is
rectangular the `generalized_inverse(m)` satisfies $m m^+ m = m$ where
$m^+$ is the output of calling `generalized_inverse(m)`. Note that the 
dimensions of `generalized_inverse(m)` are equivalent to the 
dimensions of `transpose(m)`. The `generalized_inverse` exists
for any matrix, so the matrix may be singular or less than full rank.

Even though the `generalized_inverse` exists for
...
a matrix may change rank in small pertubations around the elements of the
input matrix. For example, let $A = A^* + E$ where
...
linearly dependent row). The rank of $A$ changes as the elements in $E$ 
fluctuate. Which means the matrix $A$ is not of constant rank during
differentiation.
