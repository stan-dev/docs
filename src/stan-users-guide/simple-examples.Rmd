...
$$
y_i
  \sim \mbox{normal}(\mu + \sigma_a \times \alpha_{j[i]} - \sigma_b \times \beta_{k[i]},
                     \sigma_y),
  \mbox{ for } i=1,\dots, N
\\
...
parameters being expressed as, $a_j = \mu + \sigma_{\alpha} \times \alpha_j$.
...
$a_j=\mu + \sigma_a \times \alpha_j$, the expected average rating that we
...
\mbox{sign}(z_1-z_2) \times \sqrt{|z_1-z_2|}$, and the data model is: $y_i
...
\mbox{normal}(\mu + b \times \mbox{prior\_score}_j, \sigma_a)$, which we
write as $a_j=\mu+b \times \mbox{prior\_score}_j+\sigma_a \times \alpha_j$, with
...
model is, $$a_j = b \times \mbox{prior\_score}_j+\sigma_a \times \alpha_j, \mbox{
...
> The file `data/Series1000.txt` contains 1000 time series. Each
  ...
  individual trend was $1^{\circ}$/century (in magnitude) - which is greater
...
$$
y_i = ae^{-bx_i} \times \epsilon_i, \mbox{ for } i=1,\dots,N\\
...
multiplicative:
$$
y_i = (a_1e^{-b_1x_i} + a_2e^{-b_2x_i}) \times \epsilon_i, \mbox{ for } i=1,\dots,N,
$$
