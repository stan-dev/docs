...
The upside to this is that as long as reasonable tolerances are provided
and an ODE solver well-suited to the problem is chosen, the choice of timestep
can be abstracted away. The downside is that it is not always clear from the
outset what reasonable tolerances are, or which ODE solver is best suited
to each problem. Additionally, in the course of working with an ODE model in
Stan the correct answer may change (for instance, the ODE may change).

With this in mind, the four solvers are `rk45`, `bdf`, `adams`, and
`ckrk`. Knowing nothing about the ODE, start with the `rk45` solver. There
is a list below with more information on when each solver is useful.
...
then the advanced interface may be useful. For information on choosing
tolerances, see the [control parameters section](#control-ode.section).

The four solvers in Stan are:
