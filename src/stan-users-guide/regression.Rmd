...
distributed as in the original model, since

$$
\mathbb{V}[\beta] = \mathrm{diag}(\tau)\,\Omega_L\,\mathbb{V}[z]\,(\mathrm{diag}(\tau)\,\Omega_L)' = \mathrm{diag}(\tau)\,\Omega\,\mathrm{diag}(\tau) = \Sigma.
$$

Omitting the remaining data declarations, which are the same as before, the
...
transformed parameters {
  ...
  beta = u * gamma + z * diag_pre_multiply(tau, L_Omega) * z;
}
model {
  to_vector(z) ~ std_normal();
  L_Omega ~ lkj_corr_cholesky(2);
  to_vector(gamma) ~ normal(0, 5);
  y ~ normal(columns_dot_product(x', beta[, jj]), sigma);
}
```
