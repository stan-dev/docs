...
<!-- HTML version of the figure -->
```{=html}
<table><tr><td>
\[
y = \begin{bmatrix}
0 & 1 & \mathsf{NA} & 1 \\
0 & \mathsf{NA} & \mathsf{NA} & 1 \\
\mathsf{NA} & 0 & \mathsf{NA} & \mathsf{NA}
\end{bmatrix}
\]
</td><td>
<table style="width:auto">
  <thead>
    <tr><th>\(jj\)</th><th>\(kk\)</th><th>\(y\)</th></tr>
  </thead>
  <tbody>
...
On the left is a definition of a sparse matrix \(y\) using the NA
notation from R (which is not supported by Stan).  On the right is a
database-like encoding of the same sparse matrix \(y\) that can be
used directly in Stan.  The first two columns, \(jj\) and \(kk\),
denote the indexes and the final column, \(y\), the value.  For
example, the fifth row of the database-like data structure on the
right indicates that \(y_{2,4} = 1\).
```

<!-- LaTeX version of the figure is here -->
\begin{figure}
\begin{center}
...
\end{figure}
<a name="id:sparse-data.figure"></a>
...
<a name="id:ragged-data.figure"></a>
\begin{align*}
y_1 &= \begin{bmatrix}1.3 & 2.4 & 0.9\end{bmatrix} \\
y_2 &= \begin{bmatrix}-1.8 & -0.1\end{bmatrix} \\
y_3 &= \begin{bmatrix}12.9 & 18.7 & 42.9 & 4.7\end{bmatrix} \\
\\
z &= \begin{bmatrix}1.3 & 2.4 & 0.9 & -1.8 & -0.1 & 12.9 & 18.7 & 42.9 & 4.7\end{bmatrix} \\
s &= \begin{Bmatrix}3 & 2 & 4\end{Bmatrix}
\end{align*}

On the top is the definition of a ragged data structure $y$ with three
rows of different sizes ($y_1$ is size 3, $y_2$ size 2, and $y_3$ size
4).  The bottom two lines is how to code the data in Stan, using a
single vector $y$ to hold all the values and a separate array of
integers $s$ to hold the group row sizes.  In this example, $y_1 =
z_{1:3}$, $y_2 = z_{4:5}$, and $y_3 = z_{6:9}$.

Suppose the model is a  simple varying intercept model, which,
using vectorized notation, would yield a likelihood
$$
\prod_{n=1}^3 \log \mathsf{normal}(y_n \mid \mu_n, \sigma).
$$
