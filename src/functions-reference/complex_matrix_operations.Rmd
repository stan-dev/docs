...
Stan's fast fourier transforms are set up with invertible scaling so
that the following identities hold for complex vectors `u` and `v`,

```stan
fft(inv_fft(u)) == u        inv_fft(fft(v)) == v
```

and for complex matrices `A` and `B`,
...
This is done with invertible scaling so that `inv_fft(fft(v)) == v`.
...
to each row and then to each column.  This is done with invertible
scaling so that `inv_fft2(fft2(m)) == m`.
`r since("2.30")`
...
vector `u`.  This is done with invertible scaling so that
`fft(inv_fft(u)) == u`.  If $u \in \mathbb{C}^N$ is a complex vector
with $N$ elements and $v = \textrm{fft}^{-1}(u)$, then
\[
v_n =  \sum_{m < n}
v_m \cdot
\exp\left(\frac{n \cdot m \cdot 2 \cdot \pi \cdot \sqrt{-1}}{N}).
\]
This only differs from the FFT by the sign inside the exponential.
`r since("2.30")`
...
column. This is done with invertible scaling so that
`fft2(inv_fft2(m)) = m`.
`r since("2.30")`
