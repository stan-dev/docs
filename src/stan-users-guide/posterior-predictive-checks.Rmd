...
```
The replicated data variable is declared to be the same 
...
model parameters `alpha, beta`, and `sigma.`  However,
the return type of the vectorized function `normal_rng`
real array  matching the size of any of the non-scalar arguments,
as all non-scalar arguments must have matching size.
See the Stan Functions Reference section
[Evaluating Vectorized PRNG Functions](https://mc-stan.org/docs/functions-reference/vectorization.html#prng-vectorization)
for details.
