...
See [Stan's User guide chapter on parallelization](https://mc-stan.org/docs/stan-users-guide/parallelization-chapter.html) for
details on all three approaches.
...
supplied in the `make` call as shown below.

```
make STAN_THREADS=true path/to/model
```
or on Windows
```
mingw32-make STAN_THREADS=true path/to/model.exe
```
The flag can also be written to the `make/local` file.

### Running

Before running a multi-threaded model, we need to specify the maximum number of threads
the model can use for a single chain. This is done by setting the environment variable
...
```
./model sample data file=data.csv ...
```

## Multi-processing with MPI

In order to use multi-processing with MPI in a Stan model, the models must be 
rewritten to use the `map_rect` functions. By using MPI, the model can be 
parallelized across multiple cores or a cluster. MPI with Stan is supported 
on MacOS and Linux.
...
The Stan model compiled with `STAN_MPI` is then run using an MPI launcher (typically `mpiexec` or `mpirun`).
The launcher is supplied the path to the built executable and the number of processes to start (`-n`).

Example for running a model with six processes:
```
mpiexec -n 6 path/to/model sample data file=data.csv ...
```
...
```
make STAN_OPENCL=true path/to/model
```
or on Windows
```
mingw32-make STAN_OPENCL=true path/to/model.exe
```
The flag can also be written to the `make/local` file.

### Running

The Stan model compiled with `STAN_OPENCL` must also be supplied the OpenCL platform and device IDs.
These IDs determine the device on which to run the OpenCL-supported functions on. You can list
the devices on your system using the `clinfo` program. If the system has one GPU and no OpenCL CPU
runtime, the platform and device IDs of the GPU are typically `0`.

We then supply these IDs when starting the executable as shown below:
```
path/to/model sample data file=data.csv opencl platform=0 device=1
```