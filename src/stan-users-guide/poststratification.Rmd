...
*post*stratification [@Little:1993].  Poststratification can be
...
of say 10km by 10km "squares" (they're not quite square because the
...
model of soil carbon is then estimated using poststratification by
...
which is why `N` was declared as a vector.    
...
stratum is to use a regression model.  Each demographic
feature will require a regression coefficient for each of its
subgroups, but now the parameters grow multiplicatively rather than
exponentially.  For example, with 4 age brackets, 2 sexes, 5 income
brackets, and 50 states of residence, there are only $4 + 2 + 5 + 50 =
61$ regression coefficients to estimate.  For instance, suppose that
...
$\textrm{state}[n]$ their state of residence.  If all of these
coefficients are given fixed priors, the result is a regression and
...
If there are large numbers of demographic features, but not
necessarily large numbers of any individual feature.  For instance,
even in a poll of 10,000 respondents, if they are divided by the 50
states, that's only 200 respondents per state.  When data sizes are
small, parameter estimation can be stabilized and sharpened by
providing hierarchical priors.  With hierarchical priors, the data
determines both the amount of partial pooling among the groups.  The
only drawback is that if the number of groups is small, it can be hard
...
and poststratification consist of a a sum of intercepts that vary by
...
This non-identifiability that is only mitigated by the (hierarchical)
priors.  When demographic partitions are small, as they are with
several categories in the example, it can be more computationally
tractable to enforce a sum-to-zero constraint on the coefficients.
Other values than zero will be absorbed into the intercept.  With
binary features, coefficients will be negations of each other.  For
...
concentrates the sum near zero.  
...
An alternative coding that preserves symmetry is to reduce the degrees
of freedom by defining
$$
\gamma_2 = -\gamma_1.
$$
This is straightforward in Stan.  If a prior is placed on
$\gamma_{1:2}$, the priors for $\gamma_1$ an $\gamma_2$ add together,
even though $\gamma_2 = -\gamma_1.$  The behavior of such priors can
be examined using prior predictive checks.

None of this has anything to do with the poststratification by the
way---this is just to fit a large multilevel regression with multiple
discrete categories.  Having multiple intercepts always leads to weak
non-identifiability, even with the priors on the intercepts all
centered at zero.
...
generated quantities using population group from data variable `P`.
```
data {
  int<lower = 0> N;
...
group.  Here, there are $4 \times 5 \times 2 \times 50 = 2000$
...
Finally, a prior is needed for
the coefficient in the model block,
```
epsilon ~ normal(0, 2);
```
...
the poststratification formula matches the likelihood formula;  they
