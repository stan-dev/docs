...
// ...
  ...
  target += laplace_marginal(ll_function, (a, y), f_init,
                             cov_function, (rho, alpha, x, N, delta),
                             tol, max_num_steps, hessian_block_size,
...
```
For details about the control parameters, see  [@Margossian:2022]


Stan also provides support for a limited menu of built-in functions, including
the Poisson distribution with a log link and and prior mean $a$. When using such
...
approximation using `laplace_marginal_tol_poisson_2_log_lpmf` and
`laplace_latent_tol_poisson_2_log_nrg`.
