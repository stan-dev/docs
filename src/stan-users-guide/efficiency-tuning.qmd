...
and an $J$-length vector of slope coefficients $\beta$ can be expressed as
$$
y_n = \alpha + X_{n, \cdot} \beta  + \epsilon_n,
$$
where
$$
...
$$
\mu_j = \frac{1}{N} \sum_{n=1}^{N} x_{n, j}
$$
where $x_{ij}$ is the element in the $i$-th row and $j$-th column of $X$.


The standard deviation $s_j$ of column $j$ is calculated as:
$$
s_j = \sqrt{\frac{1}{N} \sum_{n=1}^{N} (x_{n, j} - \mu_j)^2}
$$

Each element $x_{i, j}$ in column $j$ is standardized by subtracting the mean $\mu_j$ of the column and dividing by the standard deviation $s_j$,
resulting in the standardized column $z_j$:

$$
z_{i, j} = \frac{x_{i, j} - \mu_j}{s_j}
$$
where $z_{i, j}$ is the standardized value corresponding to $x_{i, j}$.
