...
same return type, but the third argument onwards may change be anything.

For instance, all of these are valid ODE signatures:
...
With noisy observations of the solution it is possible in Stan to estimate
both parameters and initial conditions. As an example, this is a harmonic
oscillator example where both the system parameter `theta` and initial state
`y0` are parameters:
...
to be correlated in the state variables (e.g., with a multivariate
normal distribution), or both (e.g., with a multivariate Student-t
distribution).
