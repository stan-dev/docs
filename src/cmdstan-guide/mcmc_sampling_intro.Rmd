...
we use the Bash or DOS shell `for` loop to set up index variables needed to identify
...
```
The Bash interpreter is sensitive to spaces; the string `{1..4}` expands to `1, 2, 3, 4`,
whereas the string `{ 1..4 }` expands to `{, 1..4, }`.
Alternatively, the expression `i in {1..4}` be written `((i=1; i<=4; i++))`.
...
```
This pushes each process into the background, so that the loop can continue
...
```
>for /l %x in (1, 1, 4) do start /b bernoulli.exe sample ^
   data file=bernoulli.data.json my_data ^ output file=output_%x.csv
```
