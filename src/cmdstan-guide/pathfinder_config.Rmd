...
If False, psis resampling cannot be performed and the algorithm returns num_paths * num_draws samples.
The output will still contain any lp values used when calculating ELBO scores within LBFGS iterations.
Valid values: $\{0, 1\}$. Default is $1$ (True).
...
and have the same defaults for [optimization](#optimize_config).
