...
## Output CSV files{#pathfinder_csv}
...
Given an output file name, CmdStan adds suffixes to the base filename to distinguish among outputs from single- and multi-path runs.
For the default output file name `output.csv`, the resulting CSV files are
```
...
These CSV files have the following structure:
...
If the `output` configuration option `diagnostic_file` is present,
the resulting diagnostic file records the approximations to the target density
